...
@Repository
public class FTZ210206RepositoryImp implements FTZ210206Repository {
	...
	/* (non-Javadoc)
	 * @see com.synesoft.ftzmis.domain.repository.FTZ210101Repository#queryFtzInMsgCtlPageInput(org.springframework.data.domain.Pageable, com.synesoft.ftzmis.domain.model.FtzInMsgCtl)
	 */
	@Override
	public Page<FtzInMsgCtl> queryFtzInMsgCtlPageInput(Pageable pageable,
			FtzInMsgCtl ftzInMsgCtl) {
		return pageFtzInMsgCtl.getPage(Table.FTZ_IN_MSG_CTL, SQLMap.SELECT_Input_COUNTS,
				SQLMap.SELECT_Input_LIST, ftzInMsgCtl, pageable);
	}
	...
	public List<FtzInTxnDtl> queryFtzInTxnDtlList(FtzInTxnDtl ftzInTxnDtl) {
		return  queryDAO.executeForObjectList(Table.FTZ_IN_TXN_DTL + "." + SQLMap.SELECT_LIST, ftzInTxnDtl);
	}
	@Override
	public int deleteFtzInMsgCtl(FtzInMsgCtl ftzInMsgCtl) {
		return updateDAO.execute(Table.FTZ_IN_MSG_CTL + "." + SQLMap.DELETE_PRIMARY_KEY, ftzInMsgCtl);
	}

	@Override
	public int deleteFtzInTxnDtls(FtzInTxnDtl ftzInTxnDtl) {
		return updateDAO.execute(Table.FTZ_IN_TXN_DTL + "." + SQLMap.DELETE_MSG_ID, ftzInTxnDtl);
	}
	
	@Override
	public int insertFtzInMsgCtl(FtzInMsgCtl ftzInMsgCtl) {
		return updateDAO.execute(Table.FTZ_IN_MSG_CTL + "." + SQLMap.INSERT, ftzInMsgCtl);
	}
	...
	public int deleteFtzInTxnDtl(FtzInTxnDtl ftzInTxnDtl) {
		return updateDAO.execute(Table.FTZ_IN_TXN_DTL + "." + SQLMap.DELETE_PRIMARY_KEY, ftzInTxnDtl);
	}
	
	@Override
	public int insertFtzInTxnDtl(FtzInTxnDtl ftzInTxnDtl) {
		return updateDAO.execute(Table.FTZ_IN_TXN_DTL + "." + SQLMap.INSERT, ftzInTxnDtl);
	}
	
	@Override
	public int updateFtzInTxnDtlSelective(FtzInTxnDtl ftzInTxnDtl) {
		return updateDAO.execute(Table.FTZ_IN_TXN_DTL + "." + SQLMap.UPDATE_PRIMARY_KEY_SELECTIVE, ftzInTxnDtl);
	}
