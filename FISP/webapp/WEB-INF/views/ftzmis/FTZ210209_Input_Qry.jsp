...
//$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210209/AddQry");
...
$("#del").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (confirm(msg)) {
			...
			$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210209/Input/DelMsg");
			$("#form").submit();
		} else {
			return false;
		}
...
$("#sbm").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (confirm(msg)) {
			...
			$("#makDatetime").val(selectedRow.makDatetime);
			$("#msgStatus").val(selectedRow.msgStatus);
			$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210209/Input/SubmitMsg");
			$("#form").submit();
		} else {
			return false;
		}
...
$("#upt").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		var msg = $("#confirmMsg1").val() + $(this).html() + $("#confirmMsg2").val();
			//$("#makDatetime").val(selectedRow.makDatetime);
			showDialog('${pageContext.request.contextPath}/FTZ210209/Input/UptMsg/Init?ftzInMsgCtl.msgId=' + selectedRow.msgId, '500', '1040');
			queryFTZ210209();
			//window.location.href = '${pageContext.request.contextPath}/FTZ210209/Input/Qry?ftzInMsgCtlVO.branchId=${FTZ210209Form.ftzInMsgCtlVO.branchId }&ftzInMsgCtlVO.startDate=${FTZ210209Form.ftzInMsgCtlVO.startDate }&ftzInMsgCtlVO.endDate=${FTZ210209Form.ftzInMsgCtlVO.endDate }&ftzInMsgCtlVO.msgNo=${FTZ210209Form.ftzInMsgCtlVO.msgNo }&ftzInMsgCtlVO.msgStatus=${FTZ210209Form.ftzInMsgCtlVO.msgStatus }';
	}
});
...
$("#dtl").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		selectedRow = eval("(" + checkSelected() + ")"); 
		showDialog('${pageContext.request.contextPath}/FTZ210209/Input/Dtl/Init?ftzInMsgCtl.msgId=' + selectedRow.msgId, '500', '1040');
	}
});

//add
$("#add").click(function() {
	showDialog('${pageContext.request.contextPath}/FTZ210209/Input/AddMsg/Init', '500', '1040');
	queryFTZ210209();
	//window.location.href = '${pageContext.request.contextPath}/FTZ210209/Input/Qry?ftzInMsgCtlVO.startDate=' 
...
.bind(
		...
		function() {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210209/Input/Dtl/Init?ftzInMsgCtl.msgId=' + selected_msgId, '500', '1040');
...
<!-- title -->
<div class="page_title"><spring:message code="ftzmis.title.210209.input"/></div>
<!-- body -->
<div class="row">
	<form:form id="form" action="${pageContext.request.contextPath}/FTZ210209/AddQry" method="post" modelAttribute="FTZ210209Form" class="form-horizontal">
...
<div class="row">
    ...
    <div class="tbl_page_body">
		<table id="pageTable" class="table table-striped table-bordered table-condensed tbl_page">
			<tbody>
			<form:form id="FTZ210209Form" action="${pageContext.request.contextPath}" modelAttribute="FTZ210209Form">
