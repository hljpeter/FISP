<script type="text/javascript">
	$(function() {
		...
		if ('add' == input_flag) {
			...
			form.action = "${pageContext.request.contextPath}/FTZ210210/AddDtlSubmit";
		} else if ('upt' == input_flag) {
			var form = document.getElementById("form");
			form.action = "${pageContext.request.contextPath}/FTZ210210/UptDtlSubmit";
		}
	...
	function addRef() {
		...
		form.action = "${pageContext.request.contextPath}/FTZ210210/AddDtlInit";
		form.submit();
	}
	...
	function showDetailDtl() {
		...
		if (null == selected_seqNo || "" == selected_seqNo) {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210210/QryDtlDtl?selected_msgId='
					...
					+ selected_seqNo + '&time=' + new Date().getTime(),'500','1024');
		}
	}
	function delDetailDtl() {
		...
		if (null == selected_seqNo || "" == selected_seqNo) {
			...
			$("#balance").val($("#balance").val().replaceAll(",", ""));
			var form = document.getElementById("form");
			form.action = "${pageContext.request.contextPath}/FTZ210210/InputDtlDel";
	...
	function addDetailDtl() {
		var selected_msgId = $("#msgId").val();
		showDialog('${pageContext.request.contextPath}/FTZ210210/AddDtlDtlInit?selected_msgId='
				+ selected_msgId + '&time=' + new Date().getTime(),'500','1024');
		queryFTZ210210();
	}
	function uptDetailDtl() {
		...
		if (null == selected_seqNo || "" == selected_seqNo) {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210210/UptDtlDtlInit?selected_msgId='
					+ selected_msgId + "&selected_seqNo="
					+ selected_seqNo + '&time=' + new Date().getTime(),'500','1024');
			queryFTZ210210();
		}
	}
	function queryFTZ210210() {
		$("#selected_msgId").val($("#msgId").val());
		$("#balance").val($("#balance").val().replaceAll(",", ""));
		var form = document.getElementById("form");
		form.action = "${pageContext.request.contextPath}/FTZ210210/DtlInitReflash";
		form.submit();
	}
</script>
...
<div class="row">
	<form:form id="form"
		action="${pageContext.request.contextPath}/FTZ210210/UptDtlSubmit"
		...
		<table class="tbl_search">
			...
			<tr>
				...
				<td><t:moneyFormat type="text" id="balance"
						...
						value="${FTZ210210Form.ftzInMsgCtl.balance}"
			...
			<c:if test="${ FTZ210210Form.input_flag eq 'upt'}">
				<tr>
					<td class="label_td"><spring:message
			...
			<tr>
				<td style="text-align: center;" colspan="4"><input id="dtlSub"
					...
					value="<spring:message code="ftz.label.SAVE_MSG" />" /></td>
			</tr>
		</table>
	</form:form>
</div>
			...
			<thead>
				...
				<tr>
					...
					<th style="vertical-align: middle; text-align: center"
						width="120px"><spring:message code="ftz.label.AMOUNT" /></th>
					<th style="vertical-align: middle; text-align: center" width="50px"><spring:message
							code="ftz.label.COUNTRY_CODE" /></th>
					<th style="vertical-align: middle; text-align: center"
						width="160px"><spring:message code="ftz.label.OPP_NAME2" /></th>
					<th style="vertical-align: middle; text-align: center" width="50px"><spring:message
			...
			<tbody>
				<c:forEach var="dto1" items="${page.content}" varStatus="i">
					<tr>
						...
						<td class="vtip" style="text-align: left; width: 160px;">${dto1.oppName}</td>
...
<div class="pagination pull-right" style="margin-top: 10px;">
	<div class="rightPage">
		<util:pagination page="${page}"
			query="selected_msgId=${FTZ210210Form.ftzInMsgCtl.msgId}"  action="${pageUrl}"/>
	</div>
</div>
