<!-- 6.3.7　应收保函&备用证（210307） / 录入新增&修改 - 交易页面 -->
<script type="text/javascript">
$(function() {
var actionFlag = '${FTZ210307Form.actionFlag }';
if (actionFlag == "addTxn") {
	$(".dtl").css("display", "none");
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/AddTxn/Submit");
	...
	if (success && success != "") {
		...
		$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/AddTxn/Init");
	...
	$(".dtl").css("display", "none");
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/UptTxn/Submit");
	var success = '${successmsg }';
	if (success && success != "") {
		$("input").attr("readonly", true);
		$("input[type=button]").attr("disabled", true);
		...
		$(".date").attr("disabled", true);
		$("#sbm").attr("disabled", true);
		$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/UptTxn/Init");
	}
} 
...
<!-- title -->
<div class="page_title"><spring:message code="ftzmis.title.210307.input.txn"/></div>
<!-- body -->
<div class="row">
	<form:form id="form" action="${pageContext.request.contextPath}" method="post" modelAttribute="FTZ210307Form" class="form-horizontal">
		...
		<table class="tbl_search">
			...
			<tr>	
				...
				<td><t:moneyFormat type="text" id="amount" name="ftzOffTxnDtl.amount" value="${FTZ210307Form.ftzOffTxnDtl.amount }" format="###,###,###,###.00" dot="true" /></td>
