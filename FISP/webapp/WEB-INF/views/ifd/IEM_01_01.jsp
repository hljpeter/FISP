...
$(function() {
	   ...
	   $("#methodTable").find("tr").bind('click', function(){
			...
			form.action = '${pageContext.request.contextPath}/iem02/02/param?mid='+mid+'&alternative='+1+'&impfileIdValue='+impFileIdValue;
...
function getMethodParam(mId) {
    $.ajax({
        url: "${pageContext.request.contextPath}/iem01/01/getparam?impfileIdValue="+impFileIdValue,
        type: "get",
        dataType: "html",
        async: true,
        data: { param: value },
...
function getValue() {
	...
	form.action = "${pageContext.request.contextPath}/iem02/03/value?impfileIdValue="+impFileIdValue;
	form.target="curWindow";
	form.submit();
}
...
function getLSValue() {
	var form = document.getElementById("form1");
	form.action = "${pageContext.request.contextPath}/iem02/04/staticparam?impfileIdValue="+impFileIdValue;
	form.target="curWindow";
	form.submit();
}
...
<div class="page_title">任务管理 / 映射配置 / 数据映射配置  / 修改字段 / 表达式</div>
	...
	<tbody>
			...
			<c:forEach var="dto" items="${methodList}" varStatus="i">
				<tr>
					...
					<td class="tbl_page_td_left vtip" width="83px">${dto.methodName}</td>
	...
	<tr>
		<td align="left">
			<fieldset style="width: 98%;padding: 2px; height:98%" >
				<table>
					<tr height="20px">
						<td align="left" valign="bottom">&nbsp;&nbsp;<spring:message code="iem.title.LinshiGongshiQu"/></td>
						<td>&nbsp;</td>
						<td align="left" valign="bottom">&nbsp;&nbsp;<spring:message code="de.title.srctable"/>(${impfileIdValue})</td>
					</tr>
					<tr>
						<td style="width:65%">							     				
									...
									<div class="tbl_page_head">
										<table
											...
											<thead>
												<tr>
												<th class="tbl_page_th" width="30px"><spring:message code="button.check"/></th>
												<th class="tbl_page_th" width="120px" style="display:none;"><spring:message code="iem.label.TYPE"/></th>
												<th class="tbl_page_th" width="120px" style="display:none;"><spring:message code="iem.label.TYPE"/></th>
												<th class="tbl_page_th" width="120px"><spring:message code="de.title.srctable"/></th>
												</tr>
											</thead>
										</table>
									</div>
										...
										<table id="fileFieldListID"
											...
											<tbody>
												<c:forEach var="dto" items="${fileFieldList}" varStatus="i">
													<tr>
													    <td class="tbl_page_td_left vtip" width="30px"><input type="checkbox" name="checkbox" ></td>
													    <td class="tbl_page_td_left vtip" width="120px" style="display:none;">java.lang.String</td>
													      <td class="tbl_page_td_left vtip" width="120px" style="display:none;">java.lang.String</td>
														<td class="tbl_page_td_left vtip" width="120px">${dto.COMMENTS}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<table
	...
	<tbody>
			...
			<c:forEach var="dto" items="${methodparamList}" varStatus="i">
				<tr>
					<td class="tbl_page_td_left vtip" style="display: none"  width="120px"><input name="users[${i.index}].pid" value="${dto.pId}"  readonly="readonly"/></td>
					<td class="tbl_page_td_left vtip" width="120px"><input name="desc" value="${dto.paramDesc}" readonly="readonly"/></td>
					<td class="tbl_page_td_left vtip" width="120px"><input name="users[${i.index}].type" value="${dto.paramType}" readonly="readonly"/></td>
					<td class="tbl_page_td_left vtip" width="120px"><input name="users[${i.index}].cvalue" value="" /></td>
...
<div id="methodDes" style="display:none">方法名：${currentMethodDesc.methodName} ，方法描述：${currentMethodDesc.methodDesc} </div>
</div>
