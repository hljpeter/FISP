<script type="text/javascript">
	...
	function search() {
		...
		form.action = "${pageContext.request.contextPath}/sm03/01/search";
		form.submit();
	}
	//add button
	function add() {
		window.showModalDialog('${pageContext.request.contextPath}/sm03/02/init', window, 'dialogHeight:500px; dialogWidth: 1024px;edge: Raised; center: Yes; help: no; resizable: Yes; status: no;');
	}
	//modify button
	function modify(userid) {
		document.getElementById("userInfUserid").value = userid;
		window.showModalDialog('${pageContext.request.contextPath}/sm03/03/search?userInf.userid='+userid, window, 'dialogHeight:500px; dialogWidth: 1024px;edge: Raised; center: Yes; help: no; resizable: Yes; status: no;');
		search();
	}
	//del button
	function del(userid) {
		...
		form.action = "${pageContext.request.contextPath}/sm03/01/del";
	...
	function detailSearch(userid) {
		document.getElementById("userInfUserid").value = userid;
		window.showModalDialog('${pageContext.request.contextPath}/sm03/05/detailSearch_01?userInf.userid='+userid, window, 'dialogHeight:500px; dialogWidth: 1024px;edge: Raised; center: Yes; help: no; resizable: Yes; status: no;');
	}
</script>
<!-- tips information -->
...
<!-- body -->
<div class="row">
	<form:form id="form" action="${pageContext.request.contextPath}/sm03/01/init" method="post" modelAttribute="userInfForm" class="form-horizontal">
...
<div class="pagination pull-right" style="margin-top: 5px; margin-bottom: 0px;">
	<div class="leftPage">
		<util:pagination action="/sm03/01/search" page="${page}" query="userid=${userInfForm.userid}&username=${userInfForm.username}" />
	</div>
</div>
