...
import com.synesoft.ftzmis.domain.service.FTZ210103Service;
...
@Controller
@RequestMapping(value = "FTZ210103")
public class FTZ210103Controller {

	public Logger logger = LoggerFactory.getLogger(getClass());

	@ModelAttribute
	public FTZ210103Form setUpForm() {
		return new FTZ210103Form();
	}

	@RequestMapping("QryDtl")
	public String queryDtl(Model model, FTZ210103Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("FTU内部活动-中长期借款批量查询开始...");
		// 组装查询信息
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		// 查询数据
		FtzInMsgCtl result_FtzInMsgCtl = ftz210103Serv
		...
		if (null == result_FtzInMsgCtl) {
			...
			logger.error("查询无记录！");
			logger.info("FTU内部活动-中长期借款批量查询结束...");
			...
			Page<FtzInTxnDtl> page = ftz210103Serv.queryFtzInTxnDtlPage(
			...
			logger.info("FTU内部活动-中长期借款批量查询结束...");
			return "ftzmis/FTZ210103_Qry_Dtl";
		}
	}

	@RequestMapping("QryDtlDtl")
	public String queryDtlDtl(Model model, FTZ210103Form form) {
		logger.info("FTU内部活动-中长期借款明细查询开始...");
		// 组装查询信息
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		...
		if (null == result_FtzInTxnDtl) {
			...
			model.addAttribute(ResultMessages.info().add("w.sm.0001"));
			logger.error("查询无记录！");
			logger.info("FTU内部活动-中长期借款明细查询结束...");
			return "ftzmis/FTZ210103_Qry_Dtl";
		}
		// 有数据则进行数据转换
		...
		if (null != result_FtzInTxnDtl.getOppBankCode()
			...
			FtzBankCode result_FtzBankCode = ftz210103Serv
		...
		logger.info("FTU内部活动-中长期借款明细查询结束 ..");
		return "ftzmis/FTZ210103_Qry_Dtl_Dtl";
	}

	@RequestMapping("InputDel")
	public String delDtl(Model model, FTZ210103Form form) {
		logger.info("FTU内部活动-中长期借款查询批量删除开始...");
		...
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		List<FtzInTxnDtl> ftzInTxnDtls = ftz210103Serv
		...
		if (i < 1) {
			...
			form.setSelected_msgId("");
			logger.info("FTU内部活动-中长期借款查询批量删除结束...");
	...
	public String queryAdd(Model model, FTZ210103Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("FTU内部活动-中长期借款录入查询开始...");
		...
		query_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210103);

		// query DpMppCfg page list
		Page<FtzInMsgCtl> page = ftz210103Serv.queryFtzInMsgCtlPage(
		...
		if (page.getContent().size() > 0) {
			...
			model.addAttribute(ResultMessages.info().add("w.sm.0001"));
			logger.info("FTU内部活动-中长期借款录入查询结束...");
			return "ftzmis/FTZ210103_Input_Qry";
		}
		logger.info("FTU内部活动-中长期借款录入查询结束...");
		return "ftzmis/FTZ210103_Input_Qry";
	}
	...
	public String AddDtlInit(Model model, FTZ210103Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210103/AddDtlInit");
		return "ftzmis/FTZ210103_Input_Dtl";
	}

	@RequestMapping("AddDtlSubmit")
	public String AddDtlSubmit(Model model,
			@Validated({ FTZ210103FormAddDtl.class }) FTZ210103Form form,
			BindingResult result) {
		logger.info("FTU内部活动-中长期借款录入批量录入开始...");
		if (result.hasErrors()) {
			return "ftzmis/FTZ210103_Input_Add";
		}
		// 获取录入信息
		...
		insert_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210103);
		// 插入信息
		int i = ftz210103Serv.insertFtzInMsgCtl(insert_FtzInMsgCtl);
		if (i < 1) {
		...
		form.getFtzInMsgCtl().setSubmitDate(
				DateUtil.getFormatDateAddSprit(form.getFtzInMsgCtl()
						.getSubmitDate()));
		logger.info("FTU内部活动-中长期借款录入批量录入结束...");
		form.setInput_flag("upt");
		model.addAttribute("pageUrl", "/FTZ210103/UptDtlInit");
		return "ftzmis/FTZ210103_Input_Dtl";
	}

	@RequestMapping("DtlInitReflash")
	public String DtlInitReflash(Model model, FTZ210103Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210103Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (null == ftzInMsgCtl) {
		...
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		// 查询明细数据列表
		Page<FtzInTxnDtl> page = ftz210103Serv.queryFtzInTxnDtlPage(pageable,
				query_FtzInTxnDtl);
		if (page.getContent().size() > 0) {
			List<FtzInTxnDtl> ftzInTxnDtls = page.getContent();
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
	...
	public String UptDtlInit(Model model, FTZ210103Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("FTU内部活动-中长期借款录入批量录入更新初始化开始...");
		// 检查批量是否存在
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210103Serv
		...
		if (null == ftzInMsgCtl) {
			model.addAttribute(ResultMessages.error().add("w.sm.0001"));
			logger.info("FTU内部活动-中长期借款录入批量录入更新初始化结束...");
			return "ftzmis/FTZ210103_Input_Dtl";
		}
		ftzInMsgCtl.setSubmitDate(DateUtil.getFormatDateAddSprit(ftzInMsgCtl
				.getSubmitDate()));
	...
	public String UptDtlSubmit(Model model,
		...
		int i = ftz210103Serv.updateFtzInMsgCtl(update_FtzInMsgCtl, null);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
			form.setFtzInMsgCtl(ftz210103Serv
	...
	public String SubmitDtl(Model model, FTZ210103Form form) {
		FtzInMsgCtl ftzInMsgCtl = new FtzInMsgCtl();
		ftzInMsgCtl.setMsgId(form.getSelected_msgId());
		...
		int i = ftz210103Serv.updateFtzInMsgCtlForSubmit(ftzInMsgCtl);
	...
	public String delDtlDtl(Model model, FTZ210103Form form) {
	...
	public String AddDtlDtlInit(Model model, FTZ210103Form form) {
		...
		return "ftzmis/FTZ210103_Input_Dtl_Dtl";
	}

	@RequestMapping("AddDtlDtlSubmit")
	public String AddDtlDtlSubmit(Model model,
			@Validated({ FTZ210103FormAddDtlDtl.class }) FTZ210103Form form,
			BindingResult result) {
		if (result.hasErrors()) {
			return "ftzmis/FTZ210103_Input_Dtl_Dtl";
		}
		...
		int i = ftz210103Serv.insertFtzInTxnDtl(issert_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0006"));
		} else {
	...
	public String UptDtlDtlInit(Model model, FTZ210103Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210103Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
...
//			return "forward:/FTZ210103/UptDtlDtlInit?selected_msgId="
...
//		}
		...
		update_FtzInTxnDtl
				.setChkStatus(CommonConst.FTZ_MSG_STATUS_INPUT_COMPLETED);
		int i = ftz210103Serv.updateFtzInTxnDtlSelective(update_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
		} else {
	...
	public String queryAuthDtl(Model model, FTZ210103Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210103Serv
	...
	public String AuthDtlSubmit(Model model, FTZ210103Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210103Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
	...
	public String queryAuthDtlDtl(Model model, FTZ210103Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210103Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		...
		form.setFtzInTxnDtl(result_FtzInTxnDtl);
		return "ftzmis/FTZ210103_Auth_Qry_Dtl_Dtl";
	}

	@RequestMapping("AuthDtlDtlSubmit")
	public String AuthDtlDtlSubmit(Model model, FTZ210103Form form) {
		FtzInTxnDtl ftzInTxnDtl = new FtzInTxnDtl();
...
//			model.addAttribute(ResultMessages.error().add(
//					"e.ftzmis.210101.0026"));
//			return "forward:/FTZ210103/QryAuthDtlDtl?selected_msgId="
//					+ form.getFtzInTxnDtl().getMsgId() + "&selected_seqNo="
//					+ form.getFtzInTxnDtl().getSeqNo();
//		}
		...
		int i = ftz210103Serv.updateFtzInTxnDtlSelective(ftzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
					"e.ftzmis.210301.0008"));
		} else {
	...
	@Resource
	protected FTZ210103Service ftz210103Serv;
