...
import com.synesoft.ftzmis.app.model.FTZ210104Form;
import com.synesoft.ftzmis.app.model.FTZ210104Form.FTZ210104FormAddDtl;
...
import com.synesoft.ftzmis.domain.service.FTZ210104Service;
...
@Controller
@RequestMapping(value = "FTZ210104")
public class FTZ210104Controller {

	public Logger logger = LoggerFactory.getLogger(getClass());

	@ModelAttribute
	public FTZ210104Form setUpForm() {
		return new FTZ210104Form();
	}

	@RequestMapping("QryDtl")
	public String queryDtl(Model model, FTZ210104Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("应付及暂收款批量查询开始...");
		// 组装查询信息
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		// 查询数据
		FtzInMsgCtl result_FtzInMsgCtl = ftz210104Serv
		...
		if (null == result_FtzInMsgCtl) {
			...
			Page<FtzInTxnDtl> page = ftz210104Serv.queryFtzInTxnDtlPage(
			...
			logger.info("应付及暂收款批量查询结束...");
			return "ftzmis/FTZ210104_Qry_Dtl";
		}
	}

	@RequestMapping("QryDtlDtl")
	public String queryDtlDtl(Model model, FTZ210104Form form) {
		logger.info("应付及暂收款明细查询开始...");
		// 组装查询信息
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		...
		// 查询数据
		FtzInTxnDtl result_FtzInTxnDtl = ftz210104Serv
		...
		if (null != result_FtzInTxnDtl.getOppBankCode()
			...
			FtzBankCode result_FtzBankCode = ftz210104Serv
		...
		logger.info("应付及暂收款明细查询结束 ..");
		return "ftzmis/FTZ210104_Qry_Dtl_Dtl";
	}

	@RequestMapping("InputDel")
	public String delDtl(Model model, FTZ210104Form form) {
		logger.info("应付及暂收款查询批量删除开始...");
		...
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		List<FtzInTxnDtl> ftzInTxnDtls = ftz210104Serv
		...
		int i = ftz210104Serv.deleteFtzInMsgCtl(del_FtzInMsgCtl);
	...
	public String queryAdd(Model model, FTZ210104Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("应付及暂收款录入查询开始...");
		...
		query_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210104);

		// query DpMppCfg page list
		Page<FtzInMsgCtl> page = ftz210104Serv.queryFtzInMsgCtlPage(
		...
		if (page.getContent().size() > 0) {
			...
			model.addAttribute(ResultMessages.info().add("w.sm.0001"));
			logger.info("应付及暂收款录入查询结束...");
			return "ftzmis/FTZ210104_Input_Qry";
		}
		logger.info("应付及暂收款录入查询结束...");
		return "ftzmis/FTZ210104_Input_Qry";
	}
	...
	public String AddDtlInit(Model model, FTZ210104Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210104/AddDtlInit");
		return "ftzmis/FTZ210104_Input_Dtl";
	}

	@RequestMapping("AddDtlSubmit")
	public String AddDtlSubmit(Model model,
			@Validated({ FTZ210104FormAddDtl.class }) FTZ210104Form form,
			BindingResult result) {
		logger.info("应付及暂收款录入批量录入开始...");
		if (result.hasErrors()) {
			return "ftzmis/FTZ210104_Input_Dtl";
		}
		// 获取录入信息
		...
		insert_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210104);
		// 插入信息
		int i = ftz210104Serv.insertFtzInMsgCtl(insert_FtzInMsgCtl);
		if (i < 1) {
		...
		form.getFtzInMsgCtl().setSubmitDate(
				DateUtil.getFormatDateAddSprit(form.getFtzInMsgCtl()
						.getSubmitDate()));
		logger.info("应付及暂收款录入批量录入结束...");
		form.setInput_flag("upt");
		model.addAttribute("pageUrl", "/FTZ210104/UptDtlInit");
		return "ftzmis/FTZ210104_Input_Dtl";
	}

	@RequestMapping("DtlInitReflash")
	public String DtlInitReflash(Model model, FTZ210104Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210104Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (null == ftzInMsgCtl) {
		...
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		// 查询明细数据列表
		Page<FtzInTxnDtl> page = ftz210104Serv.queryFtzInTxnDtlPage(pageable,
				query_FtzInTxnDtl);
		if (page.getContent().size() > 0) {
			List<FtzInTxnDtl> ftzInTxnDtls = page.getContent();
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
	...
	public String UptDtlInit(Model model, FTZ210104Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("应付及暂收款录入批量录入更新初始化开始...");
		// 检查批量是否存在
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210104Serv
		...
		if (null == ftzInMsgCtl) {
			model.addAttribute(ResultMessages.error().add("w.sm.0001"));
			logger.info("应付及暂收款录入批量录入更新初始化结束...");
			return "ftzmis/FTZ210104_Input_Dtl";
		}
		ftzInMsgCtl.setSubmitDate(DateUtil.getFormatDateAddSprit(ftzInMsgCtl
				.getSubmitDate()));
	...
	public String UptDtlSubmit(Model model,
		...
		int i = ftz210104Serv.updateFtzInMsgCtl(update_FtzInMsgCtl, null);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
			form.setFtzInMsgCtl(ftz210104Serv
	...
	public String SubmitDtl(Model model, FTZ210104Form form) {
		FtzInMsgCtl ftzInMsgCtl = new FtzInMsgCtl();
		ftzInMsgCtl.setMsgId(form.getSelected_msgId());
		...
		int i = ftz210104Serv.updateFtzInMsgCtlForSubmit(ftzInMsgCtl);
	...
	public String delDtlDtl(Model model, FTZ210104Form form) {
		...
		if(CommonConst.FTZ_MSG_STATUS_AUTH_SUCC.equals(result_FtzInTxnDtl.getChkStatus())){
			...
			form.setSelected_msgId("");
			form.setSelected_seqNo(null);
			return "forward:/FTZ210104/UptDtlInit";
			
		}

		int i = ftz210104Serv.deleteFtzInTxnDtl(del_FtzInTxnDtl);
	...
	public String AddDtlDtlInit(Model model, FTZ210104Form form) {
		...
		return "ftzmis/FTZ210104_Input_Dtl_Dtl";
	}

	@RequestMapping("AddDtlDtlSubmit")
	public String AddDtlDtlSubmit(Model model,
			@Validated({ FTZ210104FormAddDtlDtl.class }) FTZ210104Form form,
			BindingResult result) {
		if (result.hasErrors()) {
			return "ftzmis/FTZ210104_Input_Dtl_Dtl";
		}
		...
		int i = ftz210104Serv.insertFtzInTxnDtl(issert_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0006"));
		} else {
	...
	public String UptDtlDtlInit(Model model, FTZ210104Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210104Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
...
//			return "forward:/FTZ210104/UptDtlDtlInit?selected_msgId="
...
//		}
		...
		update_FtzInTxnDtl
				.setChkStatus(CommonConst.FTZ_MSG_STATUS_INPUT_COMPLETED);
		int i = ftz210104Serv.updateFtzInTxnDtlSelective(update_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
		} else {
	...
	public String queryAuthDtl(Model model, FTZ210104Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210104Serv
		...
		if (null == result_FtzInMsgCtl) {
			...
			form.setSelected_msgId("");
			form.setSelected_seqNo(null);
			return "ftzmis/FTZ210104_Auth_Qry_Dtl";
		} else {
			result_FtzInMsgCtl.setSubmitDate(DateUtil
					.getFormatDateAddSprit(result_FtzInMsgCtl.getSubmitDate()));
			result_FtzInMsgCtl.setSndDatetime(DateUtil
	...
	public String AuthDtlSubmit(Model model, FTZ210104Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210104Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		...
		if (null == ftzInTxnDtls || ftzInTxnDtls.size() < 1) {
			...
			update_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210104);
			update_FtzInMsgCtl
	...
	public String queryAuthDtlDtl(Model model, FTZ210104Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210104Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		...
		form.setFtzInTxnDtl(result_FtzInTxnDtl);
		return "ftzmis/FTZ210104_Auth_Qry_Dtl_Dtl";
	}

	@RequestMapping("AuthDtlDtlSubmit")
	public String AuthDtlDtlSubmit(Model model, FTZ210104Form form) {
		FtzInTxnDtl ftzInTxnDtl = new FtzInTxnDtl();
...
//		}
		...
		int i = ftz210104Serv.updateFtzInTxnDtlSelective(ftzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
					"e.ftzmis.210301.0008"));
		} else {
	...
	@Resource
	protected FTZ210104Service ftz210104Serv;
