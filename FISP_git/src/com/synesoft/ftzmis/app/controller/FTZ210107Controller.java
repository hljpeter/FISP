...
import com.synesoft.ftzmis.app.model.FTZ210107Form;
import com.synesoft.ftzmis.app.model.FTZ210107Form.FTZ210107FormAddDtl;
import com.synesoft.ftzmis.app.model.FTZ210107Form.FTZ210107FormAddDtlDtl;
import com.synesoft.ftzmis.domain.model.FtzBankCode;
import com.synesoft.ftzmis.domain.model.FtzInMsgCtl;
import com.synesoft.ftzmis.domain.model.FtzInTxnDtl;
import com.synesoft.ftzmis.domain.service.FTZ210107Service;
...
@Controller
@RequestMapping(value = "FTZ210107")
public class FTZ210107Controller {

	public Logger logger = LoggerFactory.getLogger(getClass());

	@ModelAttribute
	public FTZ210107Form setUpForm() {
		return new FTZ210107Form();
	}

	@RequestMapping("QryDtl")
	public String queryDtl(Model model, FTZ210107Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("同业往来批量查询开始...");
		// 组装查询信息
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		// 查询数据
		FtzInMsgCtl result_FtzInMsgCtl = ftz210107Serv
		...
		if (null == result_FtzInMsgCtl) {
			...
			Page<FtzInTxnDtl> page = ftz210107Serv.queryFtzInTxnDtlPage(
			...
			logger.info("同业往来批量查询结束...");
			return "ftzmis/FTZ210107_Qry_Dtl";
		}
	}

	@RequestMapping("QryDtlDtl")
	public String queryDtlDtl(Model model, FTZ210107Form form) {
		logger.info("同业往来明细查询开始...");
		// 组装查询信息
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		...
		// 查询数据
		FtzInTxnDtl result_FtzInTxnDtl = ftz210107Serv
		...
		if (null != result_FtzInTxnDtl.getOppBankCode()
			...
			FtzBankCode result_FtzBankCode = ftz210107Serv
		...
		logger.info("同业往来明细查询结束 ..");
		return "ftzmis/FTZ210107_Qry_Dtl_Dtl";
	}

	@RequestMapping("InputDel")
	public String delDtl(Model model, FTZ210107Form form) {
		logger.info("同业往来查询批量删除开始...");
		...
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		List<FtzInTxnDtl> ftzInTxnDtls = ftz210107Serv
		...
		if (null != ftzInTxnDtls) {
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
				if (CommonConst.FTZ_MSG_STATUS_AUTH_SUCC.equals(ftzInTxnDtl
					...
					return "forward:/FTZ210107/AddQry";
				}
			}
		}
		...
		int i = ftz210107Serv.deleteFtzInMsgCtl(del_FtzInMsgCtl);
	...
	public String queryAdd(Model model, FTZ210107Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("同业往来录入查询开始...");
		...
		query_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210107);

		// query DpMppCfg page list
		Page<FtzInMsgCtl> page = ftz210107Serv.queryFtzInMsgCtlPage(
		...
		if (page.getContent().size() > 0) {
			...
			model.addAttribute(ResultMessages.info().add("w.sm.0001"));
			logger.info("同业往来录入查询结束...");
			return "ftzmis/FTZ210107_Input_Qry";
		}
		logger.info("同业往来录入查询结束...");
		return "ftzmis/FTZ210107_Input_Qry";
	}
	...
	public String AddDtlInit(Model model, FTZ210107Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210107/AddDtlInit");
		return "ftzmis/FTZ210107_Input_Dtl";
	}

	@RequestMapping("AddDtlSubmit")
	public String AddDtlSubmit(Model model,
			@Validated({ FTZ210107FormAddDtl.class }) FTZ210107Form form,
			BindingResult result) {
		logger.info("同业往来录入批量录入开始...");
		if (result.hasErrors()) {
			return "ftzmis/FTZ210107_Input_Dtl";
		}
		// 获取录入信息
		...
		insert_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210107);
		// 插入信息
		int i = ftz210107Serv.insertFtzInMsgCtl(insert_FtzInMsgCtl);
		if (i < 1) {
		...
		form.getFtzInMsgCtl().setSubmitDate(
				DateUtil.getFormatDateAddSprit(form.getFtzInMsgCtl()
						.getSubmitDate()));
		logger.info("同业往来录入批量录入结束...");
		form.setInput_flag("upt");
		model.addAttribute("pageUrl", "/FTZ210107/UptDtlInit");
		return "ftzmis/FTZ210107_Input_Dtl";
	}

	@RequestMapping("DtlInitReflash")
	public String DtlInitReflash(Model model, FTZ210107Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210107Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (null == ftzInMsgCtl) {
		...
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		// 查询明细数据列表
		Page<FtzInTxnDtl> page = ftz210107Serv.queryFtzInTxnDtlPage(pageable,
				query_FtzInTxnDtl);
		if (page.getContent().size() > 0) {
			List<FtzInTxnDtl> ftzInTxnDtls = page.getContent();
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
	...
	public String UptDtlInit(Model model, FTZ210107Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("同业往来录入批量录入更新初始化开始...");
		// 检查批量是否存在
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210107Serv
		...
		if (null == ftzInMsgCtl) {
			model.addAttribute(ResultMessages.error().add("w.sm.0001"));
			logger.info("同业往来录入批量录入更新初始化结束...");
			return "ftzmis/FTZ210107_Input_Upt";
		}
		ftzInMsgCtl.setSubmitDate(DateUtil.getFormatDateAddSprit(ftzInMsgCtl
				.getSubmitDate()));
		ftzInMsgCtl.setBalanceCode(ftzInMsgCtl.getBalanceCode().trim());
	...
	public String UptDtlSubmit(Model model,
		...
		int i = ftz210107Serv.updateFtzInMsgCtl(update_FtzInMsgCtl, null);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
			form.setFtzInMsgCtl(ftz210107Serv
	...
	public String SubmitDtl(Model model, FTZ210107Form form) {
		FtzInMsgCtl ftzInMsgCtl = new FtzInMsgCtl();
		ftzInMsgCtl.setMsgId(form.getSelected_msgId());
		...
		int i = ftz210107Serv.updateFtzInMsgCtlForSubmit(ftzInMsgCtl);
	...
	public String delDtlDtl(Model model, FTZ210107Form form) {
		...
		if(CommonConst.FTZ_MSG_STATUS_AUTH_SUCC.equals(result_FtzInTxnDtl.getChkStatus())){
			...
			form.setSelected_msgId("");
			form.setSelected_seqNo(null);
			return "forward:/FTZ210107/UptDtlInit";
			
		}

		int i = ftz210107Serv.deleteFtzInTxnDtl(del_FtzInTxnDtl);
	...
	public String AddDtlDtlInit(Model model, FTZ210107Form form) {
		...
		return "ftzmis/FTZ210107_Input_Dtl_Dtl";
	}

	@RequestMapping("AddDtlDtlSubmit")
	public String AddDtlDtlSubmit(Model model,
			@Validated({ FTZ210107FormAddDtlDtl.class }) FTZ210107Form form,
			BindingResult result) {
		if (result.hasErrors()) {
			return "ftzmis/FTZ210107_Input_Dtl_Dtl";
		}
		...
		int i = ftz210107Serv.insertFtzInTxnDtl(issert_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0006"));
		} else {
	...
	public String UptDtlDtlInit(Model model, FTZ210107Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210107Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
...
//			return "forward:/FTZ210107/UptDtlDtlInit?selected_msgId="
...
//		}
		...
		update_FtzInTxnDtl
				.setChkStatus(CommonConst.FTZ_MSG_STATUS_INPUT_COMPLETED);
		int i = ftz210107Serv.updateFtzInTxnDtlSelective(update_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
		} else {
	...
	public String queryAuthDtl(Model model, FTZ210107Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210107Serv
		...
		if (null == result_FtzInMsgCtl) {
			...
			form.setSelected_msgId("");
			form.setSelected_seqNo(null);
			return "ftzmis/FTZ210107_Auth_Qry_Dtl";
		} else {
			result_FtzInMsgCtl.setSubmitDate(DateUtil
					.getFormatDateAddSprit(result_FtzInMsgCtl.getSubmitDate()));
			result_FtzInMsgCtl.setSndDatetime(DateUtil
	...
	public String AuthDtlSubmit(Model model, FTZ210107Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210107Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (CommonConst.FTZ_MSG_STATUS_INPUTING.equals(result_FtzInMsgCtl
			...
			return "forward:/FTZ210107/QryAuthDtl";
		}
		UserInf userInfo = ContextConst.getCurrentUser();
		...
		if (null == ftzInTxnDtls || ftzInTxnDtls.size() < 1) {
			...
			update_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210107);
			update_FtzInMsgCtl
	...
	public String queryAuthDtlDtl(Model model, FTZ210107Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210107Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		...
		form.setFtzInTxnDtl(result_FtzInTxnDtl);
		return "ftzmis/FTZ210107_Auth_Qry_Dtl_Dtl";
	}

	@RequestMapping("AuthDtlDtlSubmit")
	public String AuthDtlDtlSubmit(Model model, FTZ210107Form form) {
		FtzInTxnDtl ftzInTxnDtl = new FtzInTxnDtl();
...
//			model.addAttribute(ResultMessages.error().add(
//					"e.ftzmis.210101.0026"));
//			return "forward:/FTZ210107/QryAuthDtlDtl?selected_msgId="
//					+ form.getFtzInTxnDtl().getMsgId() + "&selected_seqNo="
//					+ form.getFtzInTxnDtl().getSeqNo();
//		}
		...
		int i = ftz210107Serv.updateFtzInTxnDtlSelective(ftzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
					"e.ftzmis.210301.0008"));
		} else {
	...
	@Resource
	protected FTZ210107Service ftz210107Serv;
