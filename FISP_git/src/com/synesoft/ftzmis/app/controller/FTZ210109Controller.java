...
import com.synesoft.ftzmis.app.model.FTZ210109Form;
import com.synesoft.ftzmis.app.model.FTZ210109Form.FTZ210109FormAddDtl;
import com.synesoft.ftzmis.app.model.FTZ210109Form.FTZ210109FormAddDtlDtl;
import com.synesoft.ftzmis.domain.model.FtzBankCode;
import com.synesoft.ftzmis.domain.model.FtzInMsgCtl;
import com.synesoft.ftzmis.domain.model.FtzInTxnDtl;
import com.synesoft.ftzmis.domain.service.FTZ210109Service;
...
@Controller
@RequestMapping(value = "FTZ210109")
public class FTZ210109Controller {

	public Logger logger = LoggerFactory.getLogger(getClass());

	@ModelAttribute
	public FTZ210109Form setUpForm() {
		return new FTZ210109Form();
	}

	@RequestMapping("QryDtl")
	public String queryDtl(Model model, FTZ210109Form form,
		...
		// 查询数据
		FtzInMsgCtl result_FtzInMsgCtl = ftz210109Serv
		...
		if (null == result_FtzInMsgCtl) {
			...
			Page<FtzInTxnDtl> page = ftz210109Serv.queryFtzInTxnDtlPage(
			...
			logger.info("外汇买卖批量查询结束...");
			return "ftzmis/FTZ210109_Qry_Dtl";
		}
	}

	@RequestMapping("QryDtlDtl")
	public String queryDtlDtl(Model model, FTZ210109Form form) {
		...
		// 查询数据
		FtzInTxnDtl result_FtzInTxnDtl = ftz210109Serv
		...
		if (null != result_FtzInTxnDtl.getOppBankCode()
			...
			FtzBankCode result_FtzBankCode = ftz210109Serv
		...
		logger.info("外汇买卖明细查询结束 ..");
		return "ftzmis/FTZ210109_Qry_Dtl_Dtl";
	}

	@RequestMapping("InputDel")
	public String delDtl(Model model, FTZ210109Form form) {
		...
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		List<FtzInTxnDtl> ftzInTxnDtls = ftz210109Serv
		...
		if (null != ftzInTxnDtls) {
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
				if (CommonConst.FTZ_MSG_STATUS_AUTH_SUCC.equals(ftzInTxnDtl
					...
					return "forward:/FTZ210109/AddQry";
				}
			}
		}
		...
		int i = ftz210109Serv.deleteFtzInMsgCtl(del_FtzInMsgCtl);
	...
	public String queryAdd(Model model, FTZ210109Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("外汇买卖录入查询开始...");
		...
		query_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210109);

		// query DpMppCfg page list
		Page<FtzInMsgCtl> page = ftz210109Serv.queryFtzInMsgCtlPage(
		...
		if (page.getContent().size() > 0) {
			...
			model.addAttribute(ResultMessages.info().add("w.sm.0001"));
			logger.info("外汇买卖录入查询结束...");
			return "ftzmis/FTZ210109_Input_Qry";
		}
		logger.info("外汇买卖录入查询结束...");
		return "ftzmis/FTZ210109_Input_Qry";
	}
	...
	public String AddDtlInit(Model model, FTZ210109Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210109/AddDtlInit");
		return "ftzmis/FTZ210109_Input_Dtl";
	}

	@RequestMapping("AddDtlSubmit")
	public String AddDtlSubmit(Model model,
			@Validated({ FTZ210109FormAddDtl.class }) FTZ210109Form form,
			BindingResult result) {
		logger.info("外汇买卖录入批量录入开始...");
		if (result.hasErrors()) {
			return "ftzmis/FTZ210109_Input_Dtl";
		}
		// 获取录入信息
		...
		insert_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210109);
		// 插入信息
		int i = ftz210109Serv.insertFtzInMsgCtl(insert_FtzInMsgCtl);
		if (i < 1) {
		...
		form.getFtzInMsgCtl().setSubmitDate(
				DateUtil.getFormatDateAddSprit(form.getFtzInMsgCtl()
						.getSubmitDate()));
		logger.info("外汇买卖录入批量录入结束...");
		form.setInput_flag("upt");
		model.addAttribute("pageUrl", "/FTZ210109/UptDtlInit");
		return "ftzmis/FTZ210109_Input_Dtl";
	}

	@RequestMapping("DtlInitReflash")
	public String DtlInitReflash(Model model, FTZ210109Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210109Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (null == ftzInMsgCtl) {
		...
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		// 查询明细数据列表
		Page<FtzInTxnDtl> page = ftz210109Serv.queryFtzInTxnDtlPage(pageable,
				query_FtzInTxnDtl);
		if (page.getContent().size() > 0) {
			List<FtzInTxnDtl> ftzInTxnDtls = page.getContent();
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
	...
	public String UptDtlInit(Model model, FTZ210109Form form,
			@PageableDefaults Pageable pageable) {
		...
		if (null == ftzInMsgCtl) {
			...
			return "ftzmis/FTZ210109_Input_Upt";
		}
		ftzInMsgCtl.setSubmitDate(DateUtil.getFormatDateAddSprit(ftzInMsgCtl
				.getSubmitDate()));
		ftzInMsgCtl.setBalanceCode(ftzInMsgCtl.getBalanceCode().trim());
	...
	public String UptDtlSubmit(Model model,
		...
		int i = ftz210109Serv.updateFtzInMsgCtl(update_FtzInMsgCtl, null);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
			form.setFtzInMsgCtl(ftz210109Serv
	...
	public String SubmitDtl(Model model, FTZ210109Form form) {
		FtzInMsgCtl ftzInMsgCtl = new FtzInMsgCtl();
		ftzInMsgCtl.setMsgId(form.getSelected_msgId());
		...
		int i = ftz210109Serv.updateFtzInMsgCtlForSubmit(ftzInMsgCtl);
	...
	public String delDtlDtl(Model model, FTZ210109Form form) {
		...
		if(CommonConst.FTZ_MSG_STATUS_AUTH_SUCC.equals(result_FtzInTxnDtl.getChkStatus())){
			...
			form.setSelected_msgId("");
			form.setSelected_seqNo(null);
			return "forward:/FTZ210109/UptDtlInit";
			
		}

		int i = ftz210109Serv.deleteFtzInTxnDtl(del_FtzInTxnDtl);
	...
	public String AddDtlDtlInit(Model model, FTZ210109Form form) {
		...
		return "ftzmis/FTZ210109_Input_Dtl_Dtl";
	}

	@RequestMapping("AddDtlDtlSubmit")
	public String AddDtlDtlSubmit(Model model,
			@Validated({ FTZ210109FormAddDtlDtl.class }) FTZ210109Form form,
			BindingResult result) {
		if (result.hasErrors()) {
			return "ftzmis/FTZ210109_Input_Dtl_Dtl";
		}
		...
		// 外汇买卖类型
		if (null == issert_FtzInTxnDtl.getExchangeType()
				|| "".equals(issert_FtzInTxnDtl.getExchangeType().trim())) {
			ResultMessage resultMessage = ResultMessage
		...
		// 买入币种
		if (null == issert_FtzInTxnDtl.getBuyCurr()
				|| "".equals(issert_FtzInTxnDtl.getBuyCurr().trim())) {
			ResultMessage resultMessage = ResultMessage
		...
		// 卖出币种
		if (null == issert_FtzInTxnDtl.getSellCurr()
				|| "".equals(issert_FtzInTxnDtl.getSellCurr().trim())) {
			ResultMessage resultMessage = ResultMessage
		...
		// 买入金额
		if (null == issert_FtzInTxnDtl.getBuyAmt()) {
			ResultMessage resultMessage = ResultMessage
					.fromCode("e.ftzmis.210310.0004");
			resultMessages.add(resultMessage);
		}else{
			if (!Validator.CheckAmount(issert_FtzInTxnDtl.getBuyAmt())) {
				ResultMessage resultMessage = ResultMessage
						.fromCode("e.ftzmis.210303.0013");
				resultMessages.add(resultMessage);
			}
		...
		// 卖出金额
		if (null == issert_FtzInTxnDtl.getSellAmt()) {
			ResultMessage resultMessage = ResultMessage
					.fromCode("e.ftzmis.210310.0005");
			resultMessages.add(resultMessage);
		}else{
			if (!Validator.CheckAmount(issert_FtzInTxnDtl.getSellAmt())) {
				ResultMessage resultMessage = ResultMessage
						.fromCode("e.ftzmis.210303.0013");
				resultMessages.add(resultMessage);
			}
		...
		// 买入牌价
		if (null == issert_FtzInTxnDtl.getBuyRate()) {
			ResultMessage resultMessage = ResultMessage
		...
		int i = ftz210109Serv.insertFtzInTxnDtl(issert_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0006"));
		} else {
	...
	public String UptDtlDtlInit(Model model, FTZ210109Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210109Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
	...
	public String UptDtlDtlSubmit(Model model,
		...
		// return "forward:/FTZ210109/UptDtlDtlInit?selected_msgId="
		...
		update_FtzInTxnDtl
				.setChkStatus(CommonConst.FTZ_MSG_STATUS_INPUT_COMPLETED);
		int i = ftz210109Serv.updateFtzInTxnDtlSelective(update_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
		} else {
	...
	public String queryAuthDtl(Model model, FTZ210109Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210109Serv
	...
	public String AuthDtlSubmit(Model model, FTZ210109Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210109Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (CommonConst.FTZ_MSG_STATUS_INPUTING.equals(result_FtzInMsgCtl
			...
			return "forward:/FTZ210109/QryAuthDtl";
		}
		UserInf userInfo = ContextConst.getCurrentUser();
		...
		if (null == ftzInTxnDtls || ftzInTxnDtls.size() < 1) {
			...
			update_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210109);
			update_FtzInMsgCtl
	...
	public String queryAuthDtlDtl(Model model, FTZ210109Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210109Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		...
		form.setFtzInTxnDtl(result_FtzInTxnDtl);
		return "ftzmis/FTZ210109_Auth_Qry_Dtl_Dtl";
	}

	@RequestMapping("AuthDtlDtlSubmit")
	public String AuthDtlDtlSubmit(Model model, FTZ210109Form form) {
		FtzInTxnDtl ftzInTxnDtl = new FtzInTxnDtl();
		...
		// model.addAttribute(ResultMessages.error().add(
		// "e.ftzmis.210101.0026"));
		// return "forward:/FTZ210109/QryAuthDtlDtl?selected_msgId="
		// + form.getFtzInTxnDtl().getMsgId() + "&selected_seqNo="
		// + form.getFtzInTxnDtl().getSeqNo();
		// }
		...
		int i = ftz210109Serv.updateFtzInTxnDtlSelective(ftzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
					"e.ftzmis.210301.0008"));
		} else {
	...
	@Resource
	protected FTZ210109Service ftz210109Serv;
