...
import com.synesoft.ftzmis.app.model.FTZ210110Form;
import com.synesoft.ftzmis.app.model.FTZ210110Form.FTZ210110FormAddDtl;
import com.synesoft.ftzmis.app.model.FTZ210110Form.FTZ210110FormAddDtlDtl;
...
@Controller
@RequestMapping(value = "FTZ210110")
public class FTZ210110Controller {

	public Logger logger = LoggerFactory.getLogger(getClass());

	@ModelAttribute
	public FTZ210110Form setUpForm() {
		return new FTZ210110Form();
	}

	@RequestMapping("QryDtl")
	public String queryDtl(Model model, FTZ210110Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("FTU委托存款及委托投资基金(净)批量查询开始...");
		// 组装查询信息
		...
		if (null == result_FtzInMsgCtl) {
			...
			logger.error("查询无记录！");
			logger.info("FTU委托存款及委托投资基金(净)批量查询结束...");
			...
			logger.info("FTU委托存款及委托投资基金(净)批量查询结束...");
			return "ftzmis/FTZ210110_Qry_Dtl";
		}
	}

	@RequestMapping("QryDtlDtl")
	public String queryDtlDtl(Model model, FTZ210110Form form) {
		logger.info("FTU委托存款及委托投资基金(净)明细查询开始...");
		// 组装查询信息
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		...
		if (null == result_FtzInTxnDtl) {
			...
			model.addAttribute(ResultMessages.info().add("w.sm.0001"));
			logger.error("查询无记录！");
			logger.info("FTU委托存款及委托投资基金(净)明细查询结束...");
			return "ftzmis/FTZ210110_Qry_Dtl";
		}
		// 有数据则进行数据转换
		...
		logger.info("FTU委托存款及委托投资基金(净)明细查询结束 ..");
		return "ftzmis/FTZ210110_Qry_Dtl_Dtl";
	}

	@RequestMapping("InputDel")
	public String delDtl(Model model, FTZ210110Form form) {
		logger.info("FTU委托存款及委托投资基金(净)查询批量删除开始...");
		...
		if (null != ftzInTxnDtls) {
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
				if (CommonConst.FTZ_MSG_STATUS_AUTH_SUCC.equals(ftzInTxnDtl
					...
					return "forward:/FTZ210110/AddQry";
				}
			}
		}
		...
		if (i < 1) {
			...
			form.setSelected_msgId("");
			logger.info("FTU委托存款及委托投资基金(净)查询批量删除结束...");
			return "forward:/FTZ210110/AddQry";
		}
	...
	public String queryAdd(Model model, FTZ210110Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("FTU委托存款及委托投资基金(净)录入查询开始...");
		...
		if (page.getContent().size() > 0) {
			...
			model.addAttribute(ResultMessages.info().add("w.sm.0001"));
			logger.info("FTU委托存款及委托投资基金(净)录入查询结束...");
			return "ftzmis/FTZ210110_Input_Qry";
		}
		logger.info("FTU委托存款及委托投资基金(净)录入查询结束...");
		return "ftzmis/FTZ210110_Input_Qry";
	}
	...
	public String AddDtlInit(Model model, FTZ210110Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210110/AddDtlInit");
		return "ftzmis/FTZ210110_Input_Dtl";
	}

	@RequestMapping("AddDtlSubmit")
	public String AddDtlSubmit(Model model,
			@Validated({ FTZ210110FormAddDtl.class }) FTZ210110Form form,
			BindingResult result) {
		logger.info("FTU委托存款及委托投资基金(净)录入批量录入开始...");
		if (result.hasErrors()) {
			return "ftzmis/FTZ210110_Input_Dtl";
		}
		// 获取录入信息
		...
		form.getFtzInMsgCtl().setSubmitDate(
				DateUtil.getFormatDateAddSprit(form.getFtzInMsgCtl()
						.getSubmitDate()));
		logger.info("FTU委托存款及委托投资基金(净)录入批量录入结束...");
		form.setInput_flag("upt");
		model.addAttribute("pageUrl", "/FTZ210110/UptDtlInit");
		return "ftzmis/FTZ210110_Input_Dtl";
	}

	@RequestMapping("DtlInitReflash")
	public String DtlInitReflash(Model model, FTZ210110Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
	...
	public String UptDtlInit(Model model, FTZ210110Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("FTU委托存款及委托投资基金(净)录入批量录入更新初始化开始...");
		// 检查批量是否存在
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210110Serv
		...
		if (null == ftzInMsgCtl) {
			model.addAttribute(ResultMessages.error().add("w.sm.0001"));
			logger.info("FTU委托存款及委托投资基金(净)录入批量录入更新初始化结束...");
			return "ftzmis/FTZ210110_Input_Upt";
		}
		ftzInMsgCtl.setSubmitDate(DateUtil.getFormatDateAddSprit(ftzInMsgCtl
				.getSubmitDate()));
		ftzInMsgCtl.setBalanceCode(ftzInMsgCtl.getBalanceCode().trim());
	...
	public String SubmitDtl(Model model, FTZ210110Form form) {
		FtzInMsgCtl ftzInMsgCtl = new FtzInMsgCtl();
		ftzInMsgCtl.setMsgId(form.getSelected_msgId());
	...
	public String delDtlDtl(Model model, FTZ210110Form form) {
		...
		if(CommonConst.FTZ_MSG_STATUS_AUTH_SUCC.equals(result_FtzInTxnDtl.getChkStatus())){
			...
			form.setSelected_msgId("");
			form.setSelected_seqNo(null);
			return "forward:/FTZ210110/UptDtlInit";
	...
	public String AddDtlDtlInit(Model model, FTZ210110Form form) {
	...
	public String AddDtlDtlSubmit(Model model,
			@Validated({ FTZ210110FormAddDtlDtl.class }) FTZ210110Form form,
			BindingResult result) {
		if (result.hasErrors()) {
			return "ftzmis/FTZ210110_Input_Dtl_Dtl";
		}
	...
	public String queryAuthDtl(Model model, FTZ210110Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210110Serv
	...
	public String AuthDtlSubmit(Model model, FTZ210110Form form) {
		...
		if (CommonConst.FTZ_MSG_STATUS_INPUTING.equals(result_FtzInMsgCtl
			...
			return "forward:/FTZ210110/QryAuthDtl";
		}
		UserInf userInfo = ContextConst.getCurrentUser();
		...
		if (null == ftzInTxnDtls || ftzInTxnDtls.size() < 1) {
			...
			update_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210110);
			update_FtzInMsgCtl
	...
	public String queryAuthDtlDtl(Model model, FTZ210110Form form) {
		...
		form.setFtzInTxnDtl(result_FtzInTxnDtl);
		return "ftzmis/FTZ210110_Auth_Qry_Dtl_Dtl";
	}

	@RequestMapping("AuthDtlDtlSubmit")
	public String AuthDtlDtlSubmit(Model model, FTZ210110Form form) {
		FtzInTxnDtl ftzInTxnDtl = new FtzInTxnDtl();
