...
import com.synesoft.ftzmis.domain.service.FTZ210202Service;
...
public class FTZ210202Controller {
	...
	public String UptDtlDtlInit(Model model, FTZ210202Form form) {
		...
		FtzInTxnDtl result_FtzInTxnDtl = ftz210202Serv
		...
		if (null != result_FtzInTxnDtl.getOppBankCode()
			...
			FtzBankCode result_FtzBankCode = ftz210202Serv
	...
	public String UptDtlDtlSubmit(Model model,
		...
		int i = ftz210202Serv.updateFtzInTxnDtlSelective(update_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
					"e.ftzmis.210101.0026"));
		} else {
	...
	public String AddDtlDtlSubmit(Model model,
		...
		int i = ftz210202Serv.insertFtzInTxnDtl(issert_FtzInTxnDtl);
		if (i < 1) {
	...
	public String delDtlDtl(Model model, FTZ210202Form form) {
		...
		int i = ftz210202Serv.deleteFtzInTxnDtl(del_FtzInTxnDtl);
	...
	public String queryDtl(Model model, FTZ210202Form form,
		...
		if (null == result_FtzInMsgCtl) {
			...
			FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
			query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
			Page<FtzInTxnDtl> page = ftz210202Serv.queryFtzInTxnDtlPage(
	...
	public String DtlInitReflash(Model model, FTZ210202Form form,
		...
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210202Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (null == ftzInMsgCtl) {
		...
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		// 查询明细数据列表
		Page<FtzInTxnDtl> page = ftz210202Serv.queryFtzInTxnDtlPage(pageable,
				query_FtzInTxnDtl);
		if (page.getContent().size() > 0) {
			List<FtzInTxnDtl> ftzInTxnDtls = page.getContent();
			for (FtzInTxnDtl ftzInTxnDtl : ftzInTxnDtls) {
	...
	public String UptDtlSubmit(Model model,
		...
		int i = ftz210202Serv.updateFtzInMsgCtl(update_FtzInMsgCtl, null);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
			form.setFtzInMsgCtl(ftz210202Serv
	...
	public String SubmitDtl(Model model, FTZ210202Form form) {
		...
		int i = ftz210202Serv.updateFtzInMsgCtlForSubmit(ftzInMsgCtl);
	...
	public String AuthDtlDtlSubmit(Model model, FTZ210202Form form) {
		FtzInTxnDtl ftzInTxnDtl = new FtzInTxnDtl();
		...
		if (userInfo.getUserid().equals(form.getFtzInTxnDtl().getMakUserId())) {
			...
			return "forward:/FTZ210202/QryAuthDtlDtl?selected_msgId="
					+ form.getFtzInTxnDtl().getMsgId() + "&selected_seqNo="
					+ form.getFtzInTxnDtl().getSeqNo();
		}
		...
		int i = ftz210202Serv.updateFtzInTxnDtlSelective(ftzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
	...
	public String AuthDtlSubmit(Model model, FTZ210202Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210202Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		...
		if (null == ftzInTxnDtls || ftzInTxnDtls.size() < 1) {
			...
			update_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210202);
			update_FtzInMsgCtl
