...
import com.synesoft.ftzmis.app.model.FTZ210203Form;
import com.synesoft.ftzmis.domain.model.FtzBankCode;
import com.synesoft.ftzmis.domain.model.FtzInMsgCtl;
import com.synesoft.ftzmis.domain.model.FtzInTxnDtl;
import com.synesoft.ftzmis.domain.service.FTZ210203Service;

@Controller
@RequestMapping(value = "FTZ210203")
public class FTZ210203Controller {

	public Logger logger = LoggerFactory.getLogger(getClass());

	@ModelAttribute
	public FTZ210203Form setUpForm() {
		return new FTZ210203Form();
	}
	@RequestMapping("AddQry")
	public String queryAdd(Model model, FTZ210203Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("股权及其他投资录入查询开始...");
		...
		query_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210203);
		
		// query DpMppCfg page list
		Page<FtzInMsgCtl> page = ftz210203Serv.queryFtzInMsgCtlPage(pageable,
				query_FtzInMsgCtl);
		if (page.getContent().size() > 0) {
			...
			logger.info("股权及其他录入查询结束...");
			return "ftzmis/FTZ210203_Input_Qry";
		}
		logger.info("股权及其他投资录入查询结束...");
		return "ftzmis/FTZ210203_Input_Qry";
	}
	@RequestMapping("AddDtlInit")
	public String AddDtlInit(Model model, FTZ210203Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210203/AddDtlInit");
		return "ftzmis/FTZ210203_Input_Dtl";
	}
	@RequestMapping("AddDtlSubmit")
	public String AddDtlSubmit(Model model,FTZ210203Form form, BindingResult result) {
		logger.info("股权及其他录入批量录入开始...");
...
//		// 开户机构代码
		...
		insert_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210203);
		// 插入信息
		insert_FtzInMsgCtl.setBalanceCode(insert_FtzInMsgCtl.getBalanceCode().trim());
		int i = ftz210203Serv.insertFtzInMsgCtl(insert_FtzInMsgCtl);
		if (i < 1) {
		...
		form.getFtzInMsgCtl().setSubmitDate(
				DateUtil.getFormatDateAddSprit(form.getFtzInMsgCtl()
						.getSubmitDate()));
		logger.info("股权及其他投资录入批量录入结束...");
		form.setInput_flag("upt");
		model.addAttribute("pageUrl", "/FTZ210203/UptDtlInit");
		return "ftzmis/FTZ210203_Input_Dtl";
	}
	//点击修改批量
	@RequestMapping("UptDtlInit")
	public String UptDtlInit(Model model, FTZ210203Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("股权及其他投资录入/修改批量录入更新初始化开始...");
		// 检查批量是否存在
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210203Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (null == ftzInMsgCtl) {
			model.addAttribute(ResultMessages.error().add("w.sm.0001"));
			logger.info("股权及其他投资录入/修改批量录入更新初始化结束...");
			return "ftzmis/FTZ210203_Input_Dtl";
		}
		...
		// 查询明细数据列表
		Page<FtzInTxnDtl> page = ftz210203Serv.queryFtzInTxnDtlPage(pageable,
				query_FtzInTxnDtl);
		if (page.getContent().size() > 0) {
	...
	public String UptDtlSubmit(Model model,
			FTZ210203Form form,@PageableDefaults Pageable pageable) {
		...
		update_FtzInMsgCtl.setRsv1(DateUtil.getFormatDateTimeRemoveSpritAndColon(update_FtzInMsgCtl.getMakDatetime()));
		update_FtzInMsgCtl.setRsv2(DateUtil.getFormatDateTimeRemoveSpritAndColon(update_FtzInMsgCtl.getChkDatetime()));
		update_FtzInMsgCtl.setMakDatetime(DateUtil.getNowInputDateTime());
		update_FtzInMsgCtl.setSubmitDate(DateUtil.getFormatDateRemoveSprit(update_FtzInMsgCtl.getSubmitDate()));
		int i = ftz210203Serv.updateFtzInMsgCtl(update_FtzInMsgCtl, null);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0003"));
			form.setFtzInMsgCtl(ftz210203Serv.queryFtzInMsgCtl(update_FtzInMsgCtl));
			form.getFtzInMsgCtl().setSubmitDate(DateUtil.getFormatDateAddSprit(form.getFtzInMsgCtl().getSubmitDate()));
			FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
			query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
			// 查询明细数据列表
			Page<FtzInTxnDtl> page = ftz210203Serv.queryFtzInTxnDtlPage(
	...
	public String delDtlDtl(Model model, FTZ210203Form form) {
		...
		int i = ftz210203Serv.deleteFtzInTxnDtl(del_FtzInTxnDtl);
		...
		if (i < 1) {
			...
			form.setSelected_seqNo(null);
			return "forward:/FTZ210203/UptDtlInit";
		}
	...
	public String SubmitDtl(Model model, FTZ210203Form form) {
		FtzInMsgCtl ftzInMsgCtl = new FtzInMsgCtl();
		ftzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		List<FtzInTxnDtl> ftzInTxnDtls = ftz210203Serv
		...
		if (null != ftzInTxnDtls && ftzInTxnDtls.size() > 0) {
			for (FtzInTxnDtl dtl : ftzInTxnDtls) {
				if (CommonConst.FTZ_MSG_STATUS_AUTH_FAIL.equals(dtl
					...
					return "forward:/FTZ210203/AddQry";
				}
			}
		}
		int i = ftz210203Serv.updateFtzInMsgCtlForSubmit(ftzInMsgCtl);
		if (i == 0) {
			model.addAttribute(ResultMessages.error().add(
	...
	public String queryDtl(Model model, FTZ210203Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("股权及其他投资批量查询开始...");
		// 组装查询信息
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		// 查询数据
		FtzInMsgCtl result_FtzInMsgCtl = ftz210203Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (null == result_FtzInMsgCtl) {
			...
			form.setSelected_seqNo(null);
			logger.error("查询无记录！");
			logger.info("股权及其他批量查询结束...");
			return "ftzmis/FTZ210203_Qry";
		} else {
			// 有数据则进行数据转换，查询明细数据
			...
			logger.info("股权及其他批量查询结束...");
			return "ftzmis/FTZ210203_Qry_Dtl";
		}
	}
	//删除批量
	@RequestMapping("InputDel")
	public String delDtl(Model model, FTZ210203Form form) {
		logger.info("股权及其他投资查询批量删除开始...");
		...
		int i = ftz210203Serv.deleteFtzInMsgCtl(del_FtzInMsgCtl);
	...
	public String AddDtlDtlInit(Model model, FTZ210203Form form) {
		...
		return "ftzmis/FTZ210203_Input_Dtl_Dtl";
	}
	//新增明细-明细详情保存
	@RequestMapping("AddDtlDtlSubmit")
	public String AddDtlDtlSubmit(Model model,
		 FTZ210203Form form,BindingResult result) {
		...
		int i = ftz210203Serv.insertFtzInTxnDtl(issert_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add("e.sysrunner.0006"));
		} else {
	...
	public String UptDtlDtlInit(Model model, FTZ210203Form form) {
		...
		FtzInTxnDtl result_FtzInTxnDtl = ftz210203Serv
		...
		if (null != result_FtzInTxnDtl.getOppBankCode()
			...
			FtzBankCode result_FtzBankCode = ftz210203Serv
	...
	public String UptDtlDtlSubmit(Model model, FTZ210203Form form) {
		...
		update_FtzInTxnDtl
				.setChkStatus(CommonConst.FTZ_MSG_STATUS_INPUT_COMPLETED);
		int i = ftz210203Serv.updateFtzInTxnDtlSelective(update_FtzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
	...
	public String DtlInitReflash(Model model, FTZ210203Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210203Serv
	...
	public String queryDtlDtl(Model model, FTZ210203Form form) {
		logger.info("股权及其他明细查询开始...");
		// 组装查询信息
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		query_FtzInTxnDtl.setSeqNo(form.getSelected_seqNo());
		...
		form.setFtzInTxnDtl(result_FtzInTxnDtl);
		logger.info("股权及其他投资明细查询结束 ..");
		return "ftzmis/FTZ210203_Qry_Dtl_Dtl";
	}
	//双击审核
	@RequestMapping("QryAuthDtl")
	public String queryAuthDtl(Model model, FTZ210203Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210203Serv
		...
		if (null == result_FtzInMsgCtl) {
			...
			form.setSelected_msgId("");
			form.setSelected_seqNo(null);
			return "ftzmis/FTZ210203_Auth_Qry_Dtl";
		} else {
			result_FtzInMsgCtl.setSubmitDate(DateUtil
					.getFormatDateAddSprit(result_FtzInMsgCtl.getSubmitDate()));
			result_FtzInMsgCtl.setSndDatetime(DateUtil
	...
	public String queryAuthDtlDtl(Model model, FTZ210203Form form) {
		...
		form.setFtzInTxnDtl(result_FtzInTxnDtl);
		return "ftzmis/FTZ210203_Auth_Qry_Dtl_Dtl";
	}
	//审核通过（明细详情）
	@RequestMapping("AuthDtlDtlSubmit")
	public String AuthDtlDtlSubmit(Model model, FTZ210203Form form) {
		FtzInTxnDtl ftzInTxnDtl = new FtzInTxnDtl();
		...
		if (userInfo.getUserid().equals(form.getFtzInTxnDtl().getMakUserId())) {
			...
			return "forward:/FTZ210203/QryAuthDtlDtl?selected_msgId="
		...
		int i = ftz210203Serv.updateFtzInTxnDtlSelective(ftzInTxnDtl);
		if (i < 1) {
			model.addAttribute(ResultMessages.error().add(
	...
	public String AuthDtlSubmit(Model model, FTZ210203Form form) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210203Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
		if (CommonConst.FTZ_MSG_STATUS_INPUTING.equals(result_FtzInMsgCtl
			...
			return "forward:/FTZ210203/QryAuthDtl";
		}
		UserInf userInfo = ContextConst.getCurrentUser();
		...
		if (null == ftzInTxnDtls || ftzInTxnDtls.size() < 1) {
			...
			update_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210203);
			update_FtzInMsgCtl
	...
	@Resource
	protected FTZ210203Service ftz210203Serv;
