...
import com.synesoft.ftzmis.domain.service.FTZ210205Service;

@Controller
@RequestMapping(value = "FTZ210205")
public class FTZ210205Controller {

	public Logger logger = LoggerFactory.getLogger(getClass());

	@ModelAttribute
	public FTZ210205Form setUpForm() {
		return new FTZ210205Form();
	}
	@RequestMapping("AddQry")
	public String queryAdd(Model model, FTZ210205Form form,
		...
		Page<FtzInMsgCtl> page = ftz210205Serv.queryFtzInMsgCtlPage(pageable,
				query_FtzInMsgCtl);
	...
	public String AddDtlInit(Model model, FTZ210205Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210205/AddDtlInit");
	...
	public String UptDtlInit(Model model, FTZ210205Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("买入返售资产录入批量录入更新初始化开始...");
		// 检查批量是否存在
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210205Serv
		...
		// 查询明细数据列表
		Page<FtzInTxnDtl> page = ftz210205Serv.queryFtzInTxnDtlPage(pageable,
				query_FtzInTxnDtl);
		if (page.getContent().size() > 0) {
	...
	public String delDtl(Model model, FTZ210205Form form) {
		...
		FtzInTxnDtl query_FtzInTxnDtl = new FtzInTxnDtl();
		query_FtzInTxnDtl.setMsgId(form.getSelected_msgId());
		List<FtzInTxnDtl> ftzInTxnDtls = ftz210205Serv
		...
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		// 查询数据
		FtzInMsgCtl result_FtzInMsgCtl = ftz210205Serv
				.queryFtzInMsgCtl(query_FtzInMsgCtl);
	...
	public String UptDtlSubmit(Model model,
		...
		if (i < 1) {
			...
			form.setFtzInMsgCtl(ftz210205Serv
			...
			// 查询明细数据列表
			Page<FtzInTxnDtl> page = ftz210205Serv.queryFtzInTxnDtlPage(
	...
	public String queryDtl(Model model, FTZ210205Form form,
			@PageableDefaults Pageable pageable) {
	...
	public String AddDtlDtlInit(Model model, FTZ210205Form form) {
	...
	public String DtlInitReflash(Model model, FTZ210205Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl ftzInMsgCtl = ftz210205Serv
	...
	public String AddDtlDtlSubmit(Model model, FTZ210205Form form,
			BindingResult result) {
...
//		//对方客户性质
	...
	public String UptDtlDtlInit(Model model, FTZ210205Form form) {
		...
		FtzInTxnDtl result_FtzInTxnDtl = ftz210205Serv
		...
		if (null != result_FtzInTxnDtl.getOppBankCode()
			...
			FtzBankCode result_FtzBankCode = ftz210205Serv
	...
	public String delDtlDtl(Model model, FTZ210205Form form) {
		...
		int i = ftz210205Serv.deleteFtzInTxnDtl(del_FtzInTxnDtl);
		...
		if (i < 1) {
			...
			form.setSelected_seqNo(null);
			return "forward:/FTZ210205/UptDtlInit";
		}

		return "forward:/FTZ210205/UptDtlInit";
	}
	@RequestMapping("QryAuthDtl")
	public String queryAuthDtl(Model model, FTZ210205Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210205Serv
	...
	@Resource
	protected FTZ210205Service ftz210205Serv;
