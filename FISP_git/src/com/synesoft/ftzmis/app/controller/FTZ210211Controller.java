...
/**
 * @author Tangfire
 * @date 2014-01-16 下午13:16:39
 * @version 1.0
 * @description 资金运用-库存现金(210211)
 * @system FTZMIS
 * @company 上海恩梯梯数据晋恒软件有限公司
 */

@Controller
@RequestMapping(value = "FTZ210211")
public class FTZ210211Controller {
	...
	@ModelAttribute
	public FTZ210211Form setUpForm() {
		return new FTZ210211Form();
	}
	...
	public String queryDtl(Model model, FTZ210211Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("库存现金批量查询开始...");
		// 组装查询信息
	...
	public String delDtl(Model model, FTZ210211Form form) {
		logger.info("库存现金查询批量删除开始...");
	...
	public String queryAdd(Model model, FTZ210211Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("库存现金查询开始...");
		...
		query_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210211);
	...
	public String AddDtlInit(Model model, FTZ210211Form form) {
		form.setInput_flag("add");
		model.addAttribute("pageUrl", "/FTZ210211/AddDtlInit");
		return "ftzmis/FTZ210211_Input_Dtl";
	}
	...
	public String AddDtlSubmit(Model model,
			@Validated({ FTZ210211FormAddDtl.class }) FTZ210211Form form,
			BindingResult result) {
		logger.info("库存现金批量录入开始...");
		if (result.hasErrors()) {
			return "ftzmis/FTZ210211_Input_Dtl";
		}
		// 获取录入信息
		...
		insert_FtzInMsgCtl.setMsgNo(CommonConst.MSG_NO_210211);
		...
		model.addAttribute("pageUrl", "/FTZ210211/UptDtlInit");
		...
		form.getFtzInTxnDtl().setMakDatetime(
				DateUtil.getFormatDateTimeAddSpritAndColon(form.getFtzInTxnDtl()
						.getMakDatetime()));
		logger.info("库存现金批量录入结束...");
		return "ftzmis/FTZ210211_Input_Dtl";
	}
	...
	public String UptDtlInit(Model model, FTZ210211Form form,
			@PageableDefaults Pageable pageable) {
		logger.info("库存现金录入批量录入更新初始化开始...");
		// 检查批量是否存在
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		...
		if (null == ftzInMsgCtl) {
			...
			logger.info("库存现金录入批量录入更新初始化结束...");
			return "ftzmis/FTZ210211_Input_Upt";
		}
	...
	public String SubmitDtl(Model model, FTZ210211Form form) {
	...
	public String queryAuthDtl(Model model, FTZ210211Form form,
			@PageableDefaults Pageable pageable) {
		FtzInMsgCtl query_FtzInMsgCtl = new FtzInMsgCtl();
		query_FtzInMsgCtl.setMsgId(form.getSelected_msgId());
		FtzInMsgCtl result_FtzInMsgCtl = ftz210211Serv
	...
	public String AuthDtlSubmit(Model model, FTZ210211Form form) {
		...
		if ("1".equals(form.getAuthStat())) {
			...
			ret = ftz210211Serv.updateFtzInMsgForAudit(update_FtzInMsgCtl,update_ftzInTxnDtl);
		} else if ("0".equals(form.getAuthStat())) {
			update_ftzInTxnDtl.setChkStatus(CommonConst.FTZ_MSG_STATUS_AUTH_FAIL);
			ret = ftz210211Serv.updateFtzInMsgCtlAndTxnDtl(update_FtzInMsgCtl,update_ftzInTxnDtl);
		}
		if (ret < 1) {
			model.addAttribute(ResultMessages.error().add(
