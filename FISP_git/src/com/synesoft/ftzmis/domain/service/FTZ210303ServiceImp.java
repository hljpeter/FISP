 ...
 * @date 2014-01-03
 */
@Service("ftz210303Service")
public class FTZ210303ServiceImp extends FTZOffCommonServiceImp {
	private static final Logger log = LoggerFactory.getLogger(FTZ210303ServiceImp.class);

	private String msgNo;

	public FTZ210303ServiceImp() {
		super.funcId = CommonConst.FUNCTION_FTZ_Add_210303;
		msgNo = CommonConst.MSG_NO_210303;
	}
	...
	protected void addMsgLogic(FtzOffMsgCtl ftzOffMsgCtl) {
		log.debug("FTZ210303ServiceImp.addMsgLogic() start ...");
	...
	protected void updateMsgLogic(FtzOffMsgCtl ftzOffMsgCtl) {
		log.debug("FTZ210303ServiceImp.updateMsgLogic() start ...");
	}
	...
	protected void deleteMsgLogic(FtzOffMsgCtl ftzOffMsgCtl) {
		log.debug("FTZ210303ServiceImp.deleteMsgLogic() start ...");
	...
	protected void addTxnLogic(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210303ServiceImp.addTxnLogic() start ...");
		...
		if (!Validator.CheckDistrictCode(countryCode, districtCode)) {
			log.error("[e.ftzmis.210303.0011] DebtDistrictCode cannot be empty!"); 
			messages.add("e.ftzmis.210303.0011");								
		...
		if (!Validator.CheckAccOrgCode(orgInfRepository, accOrgCode)) {
			log.error("[e.ftzmis.210303.0012] AccOrgCode is not belong to this bank!"); 
			messages.add("e.ftzmis.210303.0012");								
			throw new BusinessException(messages);
		}
		...
		if (!Validator.CheckAmount(amount)) {
			log.error("[e.ftzmis.210303.0013] The format of amount is wrong!"); 
			messages.add("e.ftzmis.210303.0013");								
			throw new BusinessException(messages);
		}
	...
	protected void updateTxnLogic(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210303ServiceImp.updateTxnLogic() start ...");
	...
	protected void deleteTxnLogic(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210303ServiceImp.deleteTxnLogic() start ...");
	}
	...
	protected void validateTxn(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210303ServiceImp.validateTxn() start ...");

		ResultMessages messages = ResultMessages.error();
		// 申报日期不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getSubmitDate())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0001");
			messages.addAll(msg);
		}
		// 所属机构代码不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getAccOrgCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0002");
			messages.addAll(msg);
		}
		// 金额不能为空
		if (ftzOffTxnDtl.getAmount() == null) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0003");
			messages.addAll(msg);
		}
		// 货币不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getCurrency())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0004");
			messages.addAll(msg);
		}
		// 境内外对手行代码不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getAccOrgCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0005");
			messages.addAll(msg);
		}
		// 债务人名称不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getSwiftCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0006");
			messages.addAll(msg);
		}
		// 债务人机构代码不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getInstitutionCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0007");
			messages.addAll(msg);
		}
		// 债务人国别代码不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getCountryCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0008");
			messages.addAll(msg);
		}
		// 保函到期日不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getExpirationDate())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0009");
			messages.addAll(msg);
		}
		// 保函类型不能为空
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getLgType())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210303.0010");
			messages.addAll(msg);
		}				
