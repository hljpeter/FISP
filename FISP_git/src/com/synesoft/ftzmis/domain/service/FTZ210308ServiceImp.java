 ...
 * @date 2014-01-07
 */
@Service("ftz210308Service")
public class FTZ210308ServiceImp extends FTZOffCommonServiceImp {
	private static final Logger log = LoggerFactory.getLogger(FTZ210308ServiceImp.class);
	
	private String msgNo;

	public FTZ210308ServiceImp() {
		super.funcId = CommonConst.FUNCTION_FTZ_Add_210308;
		msgNo = CommonConst.MSG_NO_210308;
	}
	...
	public void addMsgLogic(FtzOffMsgCtl ftzOffMsgCtl) {
		log.debug("FTZ210308ServiceImp.addMsgLogic() start ...");
	...
	protected void updateMsgLogic(FtzOffMsgCtl ftzOffMsgCtl) {
		log.debug("FTZ210308ServiceImp.updateMsgLogic() start ...");
	}
	...
	protected void deleteMsgLogic(FtzOffMsgCtl ftzOffMsgCtl) {
		log.debug("FTZ210308ServiceImp.deleteMsgLogic() start ...");
	}
	...
	protected void deleteTxnLogic(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210308ServiceImp.deleteTxnLogic() start ...");
	}
	...
	protected void addTxnLogic(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210308ServiceImp.addTxnLogic() start ...");
		...
		if (!Validator.CheckDistrictCode(countryCode, districtCode)) {
			log.error("[e.ftzmis.210308.0013] ReceiptorDistrictCode cannot be empty!"); 
			messages.add("e.ftzmis.210308.0013");								
		...
		if (!Validator.CheckAccOrgCode(orgInfRepository, accOrgCode)) {
			log.error("[e.ftzmis.210308.0011] AccOrgCode is not belong to this bank!"); 
			messages.add("e.ftzmis.210308.0011");								
			throw new BusinessException(messages);
		}
		...
		if (!Validator.CheckAmount(amount)) {
			log.error("[e.ftzmis.210308.0012] The format of amount is wrong!"); 
			messages.add("e.ftzmis.210308.0012");								
			throw new BusinessException(messages);
		}
	...
	protected void updateTxnLogic(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210308ServiceImp.updateTxnLogic() start ...");
	...
	protected void validateTxn(FtzOffTxnDtl ftzOffTxnDtl) {
		log.debug("FTZ210308ServiceImp.validateTxn() start ...");

		ResultMessages messages = ResultMessages.error();
		// 申报日期
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getSubmitDate())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0001");	
			messages.addAll(msg);
		}
		// 所属机构代码
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getAccOrgCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0002");								
			messages.addAll(msg);
		}
		// 金额
		if (ftzOffTxnDtl.getAmount() == null) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0003");								
			messages.addAll(msg);
		}
		// 货币
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getCurrency())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0004");								
			messages.addAll(msg);
		}
		// 境内外对手行代码
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getSwiftCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0005");								
			messages.addAll(msg);
		}
		// 受益人机构代码
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getInstitutionCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0006");								
			messages.addAll(msg);
		}
		// 受益人名称
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getAccountName())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0007");								
			messages.addAll(msg);
		}
		// 受益人国别代码
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getCountryCode())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0008");								
			messages.addAll(msg);
		}
		// 保兑到期日
		if (!StringUtil.isNotTrimEmpty(ftzOffTxnDtl.getExpirationDate())) {
			ResultMessage msg = ResultMessage.fromCode("e.ftzmis.210308.0009");								
			messages.addAll(msg);
		}
