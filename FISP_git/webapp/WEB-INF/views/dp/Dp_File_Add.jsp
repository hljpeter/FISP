...
$(function() {
	...
	if (msg && msg != "") {
		...
		$(".btn:not('#close')").attr("disabled", true);
	} else {
	...
	$("#fieldtbl select").live('change', function() {
		...
		var str = "";
		$("#fieldtbl select").each(function(i, obj) {
			str = str + ($(obj).val() + ",");
		});
		$("#flagJson").val(str);
	});
	...
	var addNewLine = function() {
		...
		var html = '<tr id="tr_' + count + '">'
        	...
        	+ '<td class="tbl_page_td_left" width="180px"><input type="text" name="dpFileDtlList[' + count + '].fieldName" class="span3" onkeyup="numberStringFormat(this);" onbeforepaste="numberStringFormatCopy(this);"/></td>'
        	+ '<td class="tbl_page_td_left" width="250px"><input type="text" name="dpFileDtlList[' + count + '].fieldDesc" class="span4"/></td>'
        	+ '<td class="tbl_page_td_left" width="60px"><input type="text" name="dpFileDtlList[' + count + '].fieldLen" class="span1" style="width: 60px;" onkeyup="numberFormat(this);" onbeforepaste="numberFormatCopy(this);"/></td>'
        	+ '<td class="tbl_page_td_left" width="100px"><form:select path="dpFileAddForm.dtljson" style="width: 100px"><option value=""></option><form:options  items="${DP_FILE_CUTFLAG}"/></form:select><input id="test" type="hidden" name="dpFileDtlList[' + count + '].cutFlag"></td>'
        	+ '<td class="tbl_page_td_left" width="40px"><div align="center"><button class="btn btn-small"><spring:message code='button.lable.Del'/></button></div></td>'
			+ '</tr>';
		if (count == 0) {
...
<div id="id_showMsg" style="display: none"> 
	...
	<div id="id_result">
		...
		<spring:hasBindErrors name="dpFileAddForm">
			<form:form commandName="dpFileAddForm">
...
<!-- title -->
<div class="page_title"><spring:message code="fisp.titls.dp.file.dpFielAdd"/></div>
<!-- body -->
<form:form id="form" action="${pageContext.request.contextPath}/DP_File_Add/Add" modelAttribute="dpFileAddForm">
<form:hidden path="flagJson" id="flagJson"/>
<div class="row" style="margin-bottom: 40px;">
	<table class="tbl_search">
    	...
    	<tr>
			...
			<td width="30%">
				<form:input path="dpFile.fileName" type="text" class=".input-medium" maxLength="128"/>
			</td>
		...
		<tr>
			...
			<td colspan="3" width="80%"><form:input path="dpFile.comments" type="text" class="input-xxlarge" maxLength="255"/></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="label_td"><font color="red">*</font><spring:message code="fisp.label.common.ioFlag"/></td>
			<td>
...
<div class="row" style="margin-bottom: 40px;">
    ...
    <div class="tbl_page_body" style="overflow-y:scroll">
		<table id="fieldtbl" class="table table-striped table-bordered table-condensed tbl_page" width="100%">
	        <c:forEach var="dto" items="${dpFileAddForm.dpFileDtlList}" varStatus="i">
				<tr id="tr_${i.index }">
		            ...
		            <td class="tbl_page_td_left" width="100px">
			            <form:select id="cutFlag" path="dpFileDtlList[${i.index }].cutFlag" style="width: 100px;">
							<form:options items="${DP_FILE_CUTFLAG}" />
						</form:select>
		            </td>
		            <td class="tbl_page_td_left" width="40px">
			           	<div align="center">
			           		<input type="button" id="delete" class="btn btn-small" value="<spring:message code="button.lable.Del"/>">
		               	</div>
		            </td>
				</tr>
	        </c:forEach>
