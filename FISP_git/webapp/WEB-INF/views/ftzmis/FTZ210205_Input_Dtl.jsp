<script type="text/javascript">
	$(function() {
		...
		if ('add' == input_flag) {
			...
			form.action = "${pageContext.request.contextPath}/FTZ210205/AddDtlSubmit";
		} else if ('upt' == input_flag) {
			var form = document.getElementById("form");
			form.action = "${pageContext.request.contextPath}/FTZ210205/UptDtlSubmit";
		}
				...
				.bind(
						...
						function() {
							...
							var selected_seqNo = $(this).find("td:eq(9)")
									.text();
							showDialog('${pageContext.request.contextPath}/FTZ210205/QryDtlDtl?selected_msgId='
									+ selected_msgId
									+ "&selected_seqNo="
	...
	function addRef() {
		...
		form.action = "${pageContext.request.contextPath}/FTZ210205/AddDtlInit";
		form.submit();
	}
	...
	function delDetailDtl() {
		...
		if (null == selected_seqNo || "" == selected_seqNo) {
			...
			$("#balance").val($("#balance").val().replaceAll(",", ""));
			var form = document.getElementById("form");
			form.action = "${pageContext.request.contextPath}/FTZ210205/InputDtlDel";
	...
	function addDetailDtl() {
		var selected_msgId = $("#msgId").val();
		showDialog('${pageContext.request.contextPath}/FTZ210205/AddDtlDtlInit?selected_msgId='
	...
	function uptDetailDtl() {
		...
		if (null == selected_seqNo || "" == selected_seqNo) {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210205/UptDtlDtlInit?selected_msgId='
					+ selected_msgId + "&selected_seqNo="
					+ selected_seqNo,'500','1024');
	...
	function queryFTZ210205() {
		...
		$("#balance").val($("#balance").val().replaceAll(",", ""));
		var form = document.getElementById("form");
		form.action = "${pageContext.request.contextPath}/FTZ210205/DtlInitReflash?page.page="+${page.number+1};
		form.submit();
	}
</script>
...
<div class="page_title">
	<spring:message code="ftzmis.title.210205.input.dtl" />
</div>

<div class="row">
	<form:form id="form"
		action="${pageContext.request.contextPath}/FTZ210205/UptDtlSubmit"
		...
		<input type="hidden" id="selected_chkStatus" />
			...
			<td class="label_td"><font color="red">* </font><spring:message code="ftz.label.DAILY_BALANCE" />：</td>
				<td><t:moneyFormat type="text" id="balance" name="ftzInMsgCtl.balance"
						value="${FTZ210205Form.ftzInMsgCtl.balance}"
			...
			<c:if test="${ FTZ210205Form.input_flag eq 'upt'}">
			<tr>
