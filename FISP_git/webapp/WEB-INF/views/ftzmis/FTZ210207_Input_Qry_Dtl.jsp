...
var actionFlag = '${FTZ210207Form.actionFlag }';
if (actionFlag == "addMsg") {
	$("button:not(#cls)").attr("disabled", true);
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210207/Input/AddMsg/Sumbit");
	...
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210207/Input/UptMsg/Sumbit");
	//var success = '${successmsg}';
	//if (success && success != '') {
		//$("input[type=submit]").attr("disabled", true);
	//}
	...
	//var operFlag = '${FTZ210207Form.operFlag }';
	//if (operFlag == "updated") {
		//$("input[type=submit]").attr("disabled", true);
	//}
}
...
$("#add").click(function() {
	...
	showDialog('${pageContext.request.contextPath}/FTZ210207/Input/AddTxn/Init?ftzInTxnDtl.msgId=' + $("#msgId").val(), '600', '1040');
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210207/Input/UptMsg/Init?page.page=${page.number+1}");
	$("#form").submit();
...
$("#dtl").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		showDialog('${pageContext.request.contextPath}/FTZ210207/QryDtlDtl?ftzInTxnDtl.msgId=' 
...
$("#upt").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		showDialog('${pageContext.request.contextPath}/FTZ210207/Input/UptTxn/Init?ftzInTxnDtl.msgId=' 
				+ $("#msgId").val() + '&ftzInTxnDtl.seqNo=' + selectedRow.seqNo, '600', '1040');
		$("#balance").val($("#balance").val().replaceAll(",", ""));
		$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210207/Input/UptMsg/Init?page.page=${page.number+1}");
		$("#form").submit();
...
$("#del").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (confirm(msg)) {
			...
			$("#operFlag").val("updated");
			$("#balance").val($("#balance").val().replaceAll(",", ""));
			$("#form").attr("action", '${pageContext.request.contextPath}/FTZ210207/Input/DelTxn/Submit?ftzInTxnDtl.msgId=' 
...
function accoutQry() {
	var accountNo = $("#accountNo").val();
	$.ajax({
		url : contextPath + "/FTZ210207/DtlAccountQry",
		type : "post",
		dataType : "json",
		async : false,
		data : {
...
<!-- title -->
<div class="page_title"><spring:message code="ftzmis.title.210207.input.dtl"/></div>
<!-- body -->
<div class="row">
		...
		<table class="tbl_search">
			...
			<td class="label_td"><font color="red">*</font>
				...
				<td><t:moneyFormat type="text" id="balance1"
						...
						value="${FTZ210207Form.ftzInMsgCtl.balance}"
			...
			<tr>
				...
				<c:if test="${ FTZ210207Form.actionFlag eq 'uptMsg'}">
				<tr>
					<td class="label_td"><spring:message
...
<div class="pagination pull-right" style="margin-top: 10px;">
	<div class="rightPage">
		<util:pagination page="${page}"
			query="ftzInMsgCtl.msgId=${FTZ210207Form.ftzInMsgCtl.msgId}"
			 />
	</div>
</div>
