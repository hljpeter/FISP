...
if (actionFlag == "addMsg") {
	...
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210208/Input/AddMsg/Sumbit");
	...
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210208/Input/UptMsg/Sumbit");
	//var success = '${successmsg}';
	//if (success && success != '') {
		//$("input[type=submit]").attr("disabled", true);
	//}
	...
	//var operFlag = '${FTZ210208Form.operFlag }';
	//if (operFlag == "updated") {
	//	$("input[type=submit]").attr("disabled", true);
	//}
}
...
$("#add").click(function() {
	...
	showDialog('${pageContext.request.contextPath}/FTZ210208/Input/AddTxn/Init?ftzInTxnDtl.msgId=' + $("#msgId").val(), '600', '1040');
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210208/Input/UptMsg/Init?page.page=${page.number+1}");
	$("#form").submit();
...
$("#dtl").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		showDialog('${pageContext.request.contextPath}/FTZ210208/QryDtlDtl?ftzInTxnDtl.msgId=' 
...
$("#upt").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		showDialog('${pageContext.request.contextPath}/FTZ210208/Input/UptTxn/Init?ftzInTxnDtl.msgId=' 
				+ $("#msgId").val() + '&ftzInTxnDtl.seqNo=' + selectedRow.seqNo, '600', '1040');
		$("#balance").val($("#balance").val().replaceAll(",", ""));
		$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210208/Input/UptMsg/Init?page.page=${page.number+1}");
		$("#form").submit();
...
$("#del").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (confirm(msg)) {
			...
			$("#operFlag").val("updated");
			$("#balance").val($("#balance").val().replaceAll(",", ""));
			$("#form").attr("action", '${pageContext.request.contextPath}/FTZ210208/Input/DelTxn/Submit?ftzInTxnDtl.msgId=' 
...
function accoutQry() {
	var accountNo = $("#accountNo").val();
	$.ajax({
		url : contextPath + "/FTZ210208/DtlAccountQry",
		type : "post",
		dataType : "json",
		async : false,
		data : {
...
<!-- title -->
<div class="page_title"><spring:message code="ftzmis.title.210208.input.dtl"/></div>
<!-- body -->
<div class="row">
	<form:form id="form" action="${pageContext.request.contextPath}" method="post" modelAttribute="FTZ210208Form" class="form-horizontal">
		...
		<table class="tbl_search">
			...
			<tr>
				...
				<c:if test="${ FTZ210208Form.actionFlag eq 'uptMsg'}">
				<tr>
					<td class="label_td"><spring:message
...
<div class="pagination pull-right" style="margin-top: 10px;">
	<div class="rightPage">
		<util:pagination page="${page}"
			query="ftzInMsgCtl.msgId=${FTZ210208Form.ftzInMsgCtl.msgId}"
			 />
	</div>
</div>
<div class="row">
