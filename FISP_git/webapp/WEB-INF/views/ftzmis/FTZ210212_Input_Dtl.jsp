<script type="text/javascript">
	$(function() {
		...
		if ('add' == input_flag) {
			...
			form.action = "${pageContext.request.contextPath}/FTZ210212/AddDtlSubmit";
		} else if ('upt' == input_flag) {
			var form = document.getElementById("form");
			form.action = "${pageContext.request.contextPath}/FTZ210212/UptDtlSubmit";
		}
				...
				.bind(
						...
						function() {
							...
							var selected_seqNo = $(this).find("td:eq(9)")
									.text();
							showDialog('${pageContext.request.contextPath}/FTZ210212/QryDtlDtl?selected_msgId='
									+ selected_msgId
									+ "&selected_seqNo="
	...
	function addRef() {
		...
		form.action = "${pageContext.request.contextPath}/FTZ210212/AddDtlInit";
		form.submit();
	}
	...
	function delDetailDtl() {
		...
		if (null == selected_seqNo || "" == selected_seqNo) {
			...
			$("#balance").val($("#balance").val().replaceAll(",", ""));
			var form = document.getElementById("form");
			form.action = "${pageContext.request.contextPath}/FTZ210212/InputDtlDel";
	...
	function addDetailDtl() {
		var selected_msgId = $("#msgId").val();
		showDialog('${pageContext.request.contextPath}/FTZ210212/AddDtlDtlInit?selected_msgId='
	...
	function uptDetailDtl() {
		...
		if (null == selected_seqNo || "" == selected_seqNo) {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210212/UptDtlDtlInit?selected_msgId='
					+ selected_msgId + "&selected_seqNo="
					+ selected_seqNo,'500','1024');
			queryFTZ210212();
		}
	...
	function queryFTZ210212() {
		...
		$("#balance").val($("#balance").val().replaceAll(",", ""));
		var form = document.getElementById("form");
		form.action = "${pageContext.request.contextPath}/FTZ210212/DtlInitReflash?page.page="+${page.number+1};
		form.submit();
	}
</script>
...
<div class="page_title">
	<spring:message code="ftzmis.title.210212.input.dtl" />
</div>

<div class="row">
	<form:form id="form"
		action="${pageContext.request.contextPath}/FTZ210212/UptDtlSubmit"
		...
		<table class="tbl_search">
			...
			<c:if test="${ FTZ210212Form.input_flag eq 'upt'}">
...
<div class="pagination pull-right" style="margin-top: 10px;">
	<div class="rightPage">
		<util:pagination page="${page}"
			query="selected_msgId=${FTZ210212Form.ftzInMsgCtl.msgId}"  action="${pageUrl}"/>
	</div>
</div>
...
<div class="row" style="margin-bottom: 40px;">
	<div class="navbar navbar-fixed-bottom text-center" id="footer"
		...
		<input id="add" type="button" class="btn btn-primary"
			...
			value="<spring:message code="ftz.label.DTL_DTL"/>"> 
