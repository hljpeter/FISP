...
$("#upt").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210301/Input/UptMsg/Init?page.page=" + ${page.number + 1 });
			$("#form").submit();
		}
	}
});
...
$("#del").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			if (confirm(msg)) {
				$("#operFlag").val("updated");
				$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210301/Input/DelTxn/Submit?page.page=" + ${page.number + 1 } + "&ftzOffTxnDtl.msgId=" 
	...
	<form:form id="form" action="${pageContext.request.contextPath}" method="post" modelAttribute="FTZ210301Form" class="form-horizontal">
		...
		<table class="tbl_search">
			<tr>
				...
				<td><t:dateTimeFormat type="text" value="${FTZ210301Form.ftzOffMsgCtl.workDate }" format="date" name="ftzOffMsgCtl.workDate" cssClass="input-large"/></td>
			</tr>
			<tr>	
