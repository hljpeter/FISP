<!-- 6.3.7　应收保函&备用证（210307） / 录入新增&修改 - 批量页面 -->
...
$(".tbl_page_body table tr ").dblclick(function() {
	...
	showDialog('${pageContext.request.contextPath}/FTZ210307/Input/DtlTxn/Init?ftzOffTxnDtl.msgId=' 
			+ $("#msgId").val() + "&ftzOffTxnDtl.seqNo=" + selectedRow.seqNo, '600', '1040');
});

var actionFlag = '${FTZ210307Form.actionFlag }';
if (actionFlag == "addMsg") {
	$("button:not(#cls)").attr("disabled", true);
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/AddMsg/Sumbit");
...
// add
$("#add").click(function() {
	showDialog('${pageContext.request.contextPath}/FTZ210307/Input/AddTxn/Init?ftzOffTxnDtl.msgId=' 
		+ $("#msgId").val() + '&time=' + new Date().getTime(), '600', '1040');
	
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/UptMsg/Init");
	$("#form").submit();
});
...
$("#upt").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210307/Input/UptTxn/Init?ftzOffTxnDtl.msgId=' 
					+ $("#msgId").val() + "&ftzOffTxnDtl.seqNo=" + selectedRow.seqNo + '&time=' + new Date().getTime(), '600', '1040');
				
			$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/UptMsg/Init?page.page=" + ${page.number + 1 });
			$("#form").submit();
		}
	}
});
...
$("#del").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			if (confirm(msg)) {
				$("#operFlag").val("updated");
				$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210307/Input/DelTxn/Submit?page.page=" + ${page.number + 1 } + "&ftzOffTxnDtl.msgId=" 
...
<!-- title -->
<div class="page_title"><spring:message code="ftzmis.title.210307.input.msg"/></div>
<!-- body -->
<div class="row">
	<form:form id="form" action="${pageContext.request.contextPath}" method="post" modelAttribute="FTZ210307Form" class="form-horizontal">
		...
		<table class="tbl_search">
			<tr>
				...
				<td>
					<t:dateTimeFormat type="text" value="${FTZ210307Form.ftzOffMsgCtl.workDate }" format="date" name="ftzOffMsgCtl.workDate" cssClass="input-large"/>
				</td>
			</tr>
			<tr>	
...
<div class="row" style="margin-top: 10px;">
	<div class="pagination pull-right" style="margin-top: 5px; margin-bottom: 0px;">
		<div class="leftPage">
			<util:pagination page="${page }" action="/FTZ210307/Input/UptMsg/Init" query="ftzOffMsgCtl.msgId=${FTZ210307Form.ftzOffMsgCtl.msgId }" />
		</div>
	</div>
</div>
