<!-- 6.3.8　信用证保兑（出口应收）（210308） / 录入新增&修改 - 批量页面 -->
...
$(".tbl_page_body table tr ").dblclick(function() {
	...
	showDialog('${pageContext.request.contextPath}/FTZ210308Input/DtlTxn/Init?ftzOffTxnDtl.msgId=' 
			+ $("#msgId").val() + "&ftzOffTxnDtl.seqNo=" + selectedRow.seqNo, '600', '1040');
});

var actionFlag = '${FTZ210308Form.actionFlag }';
if (actionFlag == "addMsg") {
	$("button:not(#cls)").attr("disabled", true);
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210308/Input/AddMsg/Sumbit");
	...
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210308/Input/UptMsg/Sumbit");
	$(".pboc").css("display", "");
	//var success = '${successmsg}';
	//if (success && success != '') {
	//	$("input[type=submit]").attr("disabled", true);
	...
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210308/Input/UptMsg/Sumbit");
	//var operFlag = '${FTZ210308Form.operFlag }';
	//if (operFlag == "updated") {
	//	$("input[type=submit]").attr("disabled", true);
	//}
}
...
// add
$("#add").click(function() {
	showDialog('${pageContext.request.contextPath}/FTZ210308/Input/AddTxn/Init?ftzOffTxnDtl.msgId=' 
		+ $("#msgId").val() + '&time=' + new Date().getTime(), '600', '1040');
	
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210308/Input/UptMsg/Init");
	$("#form").submit();
});
...
$("#dtl").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		showDialog('${pageContext.request.contextPath}/FTZ210308/Input/DtlTxn/Init?ftzOffTxnDtl.msgId=' 
				+ $("#msgId").val() + "&ftzOffTxnDtl.seqNo=" + selectedRow.seqNo, '600', '1040');
	}
});
...
$("#upt").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210308/Input/UptTxn/Init?ftzOffTxnDtl.msgId=' 
					+ $("#msgId").val() + "&ftzOffTxnDtl.seqNo=" + selectedRow.seqNo + '&time=' + new Date().getTime(), '600', '1040');
				
			$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210308/Input/UptMsg/Init?page.page=" + ${page.number + 1 });
			$("#form").submit();
		}
...
$("#del").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			if (confirm(msg)) {
				$("#operFlag").val("updated");
				$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210308/Input/DelTxn/Submit?page.page=" + ${page.number + 1 } + "&ftzOffTxnDtl.msgId=" 
...
<!-- title -->
<div class="page_title"><spring:message code="ftzmis.title.210308.input.msg"/></div>
<!-- body -->
<div class="row">
	<form:form id="form" action="${pageContext.request.contextPath}" method="post" modelAttribute="FTZ210308Form" class="form-horizontal">
		...
		<table class="tbl_search">
			<tr>
				...
				<td>
					<t:dateTimeFormat type="text" value="${FTZ210308Form.ftzOffMsgCtl.workDate }" format="date" name="ftzOffMsgCtl.workDate" cssClass="input-large" readonly="true"/>
				</td>
			</tr>
			<tr>	
		...
		<table class="table table-striped table-bordered table-condensed tbl_page">
			...
			<tr>
	        	<th class="tbl_page_th" width="20px"><spring:message code="ftz.label.NO"/></th>
	          	...
	          	<th class="tbl_page_th" width="120px"><spring:message code="ftz.label.RECEIPTOR_COUNTRY_CODE"/></th>
	          	<th class="tbl_page_th" width="160px"><spring:message code="ftz.label.RECEIPTOR_ENT_NAME"/></th>
...
<div class="row" style="margin-top: 10px;">
	<div class="pagination pull-right" style="margin-top: 5px; margin-bottom: 0px;">
		<div class="leftPage">
			<util:pagination page="${page }" action="/FTZ210308/Input/UptMsg/Init" query="ftzOffMsgCtl.msgId=${FTZ210308Form.ftzOffMsgCtl.msgId }" />
		</div>
	</div>
</div>
