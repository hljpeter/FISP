<!-- 6.3.9　应收银行承兑汇票（210309） / 录入新增&修改 - 批量页面 -->
...
$(".tbl_page_body table tr ").dblclick(function() {
	...
	showDialog('${pageContext.request.contextPath}/FTZ210309/Input/DtlTxn/Init?ftzOffTxnDtl.msgId=' 
...
if (actionFlag == "addMsg") {
	...
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210309/Input/AddMsg/Sumbit");
	...
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210309/Input/UptMsg/Sumbit");
	$(".pboc").css("display", "");
	//var success = '${successmsg}';
	//if (success && success != '') {
	//	$("input[type=submit]").attr("disabled", true);
...
// add
$("#add").click(function() {
	showDialog('${pageContext.request.contextPath}/FTZ210309/Input/AddTxn/Init?ftzOffTxnDtl.msgId=' 
		+ $("#msgId").val() + '&time=' + new Date().getTime(), '600', '1040');
	
	$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210309/Input/UptMsg/Init");
	$("#form").submit();
});
...
$("#upt").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			showDialog('${pageContext.request.contextPath}/FTZ210309/Input/UptTxn/Init?ftzOffTxnDtl.msgId=' 
					+ $("#msgId").val() + "&ftzOffTxnDtl.seqNo=" + selectedRow.seqNo + '&time=' + new Date().getTime(), '600', '1040');
				
			$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210309/Input/UptMsg/Init?page.page=" + ${page.number + 1 });
			$("#form").submit();
		}
	}
});
...
$("#del").click(function() {
	...
	if (!selectedRow || selectedRow == "") {
		...
		if (selectedRow.chkStatus == '03') {
			...
			if (confirm(msg)) {
				$("#operFlag").val("updated");
				$("#form").attr("action", "${pageContext.request.contextPath}/FTZ210309/Input/DelTxn/Submit?page.page=" + ${page.number + 1 } + "&ftzOffTxnDtl.msgId=" 
...
<!-- title -->
<div class="page_title"><spring:message code="ftzmis.title.210309.input.msg"/></div>
<!-- body -->
<div class="row">
	<form:form id="form" action="${pageContext.request.contextPath}" method="post" modelAttribute="FTZ210309Form" class="form-horizontal">
		...
		<table class="tbl_search">
			<tr>
				...
				<td>
					<t:dateTimeFormat type="text" value="${FTZ210309Form.ftzOffMsgCtl.workDate }" format="date" name="ftzOffMsgCtl.workDate" cssClass="input-large"/>
				</td>
			</tr>
			<tr>	
...
<div class="row" style="margin-top: 10px;">
	<div class="pagination pull-right" style="margin-top: 5px; margin-bottom: 0px;">
		<div class="leftPage">
			<util:pagination page="${page }" action="/FTZ210309/Input/UptMsg/Init" query="ftzOffMsgCtl.msgId=${FTZ210309Form.ftzOffMsgCtl.msgId }" />
		</div>
	</div>
</div>
