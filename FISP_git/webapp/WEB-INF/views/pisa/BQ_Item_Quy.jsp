...
function search() {
	...
	form.action = "${pageContext.request.contextPath}/Draft_Item/Qry";
	form.submit();
}
	//detail button
	function sheetDetail(subNo,areaType,areaCode,sheetNo,itemNo) {
		window.showModalDialog('${pageContext.request.contextPath}/Draft_Item/Detil?subNo='+subNo+'&areaType='+areaType+'&areaCode='+areaCode+'&sheetNo='+sheetNo+'&itemNo='+itemNo,window, 'dialogHeight:500px; dialogWidth: 1024px;edge: Raised; center: Yes; help: no; resizable: Yes; status: no;');
	}	
</script>
<!-- tips information -->
...
<!-- title -->
<div class="page_title"><spring:message code="pisa.draft.ItemQur"/></div>
          ...
          <tr>
            ...
            <td class="tbl_page_td_left" width="50px">
	           	<div align="center" style="height: 25px">
					<input type="button" id="detailBtn" class="btn btn-small" onclick="sheetDetail('${f:h(dto.subNo)}','${f:h(dto.areaType)}','${f:h(dto.areaCode)}','${f:h(dto.sheetNo)}','${f:h(dto.itemNo)}')"value="<spring:message code="pisa.draft.Sheet_datails"/>">
               </div>
            </td>
          </tr>
        </c:forEach>
...
<div class="pagination pull-right" style="margin-top: 5px; margin-bottom: 0px;">
	<div class="leftPage">
		<util:pagination page="${page}" action="/Draft_Item/Qry" query="itemNo=${draft_Sheet_Form.itemNo}&itemName=${draft_Sheet_Form.itemName}&subNo=${draft_Sheet_Form.subNo}&areaType=${draft_Sheet_Form.areaType}&areaCode=${draft_Sheet_Form.areaCode}" />
	</div>
</div>
